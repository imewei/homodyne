...
**Date:** October 17, 2025 **Log File:**
`/home/wei/Documents/Projects/data/C020/homodyne_results/logs/homodyne_analysis_20251017_121613.log`
...
The homodyne analysis failed with a **ValueError** when calling `curve_fit_large()`
because the code expected 3 return values (popt, pcov, info) but the NLSQ API only
returns 2 values (popt, pcov).

**Root Cause:** API incompatibility - `curve_fit_large()` does not support
`full_output=True` parameter unlike `curve_fit()`

**Impact:** 23M point dataset optimization completely blocked

**Fix Applied:** Changed unpacking from 3 values to 2 values, removed unsupported
`full_output=True` parameter
...
**Analysis:** The error recovery system attempted 3 times with parameter perturbation,
but this was futile because the error was an **API incompatibility**, not a convergence
issue.
...
**KEY FINDING:** `curve_fit_large()` returns **only 2 values**, not 3 like `curve_fit()`
...
1. ✅ Removed unsupported `full_output=True` parameter
1. ✅ Added `info = {}` to maintain consistency with `curve_fit()` code path
1. ✅ Added explanatory comment documenting the API difference
...
1. ✅ Updated message to reflect that curve_fit_large IS enabled
1. ✅ Changed second warning (1M-10M points) from WARNING to INFO level
1. ✅ Added informational message about automatic memory management
...
| Change Type | Count | |-------------|-------| | Fixed unpacking | 3 lines | | Added
comments | 4 lines | | Updated warnings | 10 lines | | **Total** | **17 lines** |
...
**Analysis:** 23 data quality warnings detected, resulting in very low quality score
(0.05/1.00)

**Status:** Non-blocking - data was still loaded and passed to optimization

**Recommendation:** Investigate data quality warnings separately (likely angle range or
...
1. There's a config file or CLI argument overriding the default
...
1. **Device Configuration:** ✅ Works (GPU with 16GB detected)
1. **Parameter Initialization:** ✅ Works (9 parameters from config)
1. **Optimization Selection:** ✅ Works (curve_fit_large selected for 23M points)
1. **GPU Memory Detection:** ✅ Works (16GB → 8GB limit)
1. **curve_fit_large Execution:** ✅ **NOW FIXED** (proper unpacking)
1. **Result Processing:** ✅ Should work (assuming convergence)

### Performance Expectations

**For 23M Point Dataset:**

| Metric | Expected Value | |--------|---------------| | Optimization method |
curve_fit_large() | | Memory limit | 8.0 GB (50% of 16GB GPU) | | Chunking | Automatic |
| Progress display | Enabled | | Estimated time | 5-7 minutes |

### Success Criteria

✅ No ValueError during unpacking ✅ Optimization completes without crashes ✅ GPU memory
stays within 8GB limit ✅ Results returned with parameters and covariance
...
1. ✅ **XLA preallocation enabled:** "Preallocation: enabled" (line 10)
1. ✅ **GPU detection working:** 16GB RTX 4090 detected correctly
1. ✅ **Data caching working:** Selective q-caching loaded in 0.541s
1. ✅ **Angle normalization working:** Angles properly normalized to [-180°, 180°]

### Areas for Future Improvement

1. **Data quality warnings:** Investigate 23 validation warnings
1. **Quality scores:** Raw data score 0.0 suggests data preprocessing needed
1. **Memory fraction override:** Check if config has explicit 0.9 setting
1. **Recovery strategy:** Improve error diagnosis to detect API errors vs convergence
   issues
...
This fix addresses an issue that was **not identified** in the validation report
because:

1. The validation report focused on configuration and performance
1. The API incompatibility only manifests at runtime
1. No tests specifically exercised the curve_fit_large() code path

**Recommendation:** Add integration test that exercises curve_fit_large() with >1M
points
...
1. ✅ XLA preallocation enabled
1. ✅ Memory fraction adjusted to 80%
1. ✅ Traceback filtering disabled
1. ✅ **NEW:** curve_fit_large API compatibility fixed
...
|---------|-------------|-------------------| | Return values | 2 or 3 (with
full_output) | Always 2 | | full_output parameter | ✅ Supported | ❌ Not supported | |
Info dict | ✅ Available | ❌ Not available | | Memory limit | ❌ Not supported | ✅
Supported | | Chunking | ❌ No | ✅ Automatic | | Progress bar | ❌ No | ✅ Optional | |
...
**Fix Applied By:** Claude Code (Homodyne Assistant) **Date:** October 17, 2025
**Status:** ✅ **CRITICAL BUG FIXED** **Code Quality:** ✅ Black + Ruff checks passing
