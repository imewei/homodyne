...
- **Optimization method**: NLSQ (fast) or MCMC (uncertainty quantification)
...
7. **Optimization**: Method selection and settings (NLSQ/MCMC)
...
- **User Guide**: :doc:`/user-guide/configuration` - Configuration concepts
...
MCMC Configuration:
- CMC (Consensus Monte Carlo) only in v2.4.1+
...
- **Configuration Version**: 2.4.1 (matches Homodyne v2.4.1)
...
This checks:
- Python version (3.12+)
- JAX installation and version (0.8.0)
...
- See :doc:`/user-guide/configuration` for configuration concepts
- See :doc:`/user-guide/cli` for command-line usage
- See :doc:`/developer-guide/index` for development setup
