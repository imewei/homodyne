...
2. Look at timestamp of most recent build
3. Click build ID to view detailed logs
4. Scroll to bottom for final status and error messages
...
2. See build status (✓ or ✗)
3. Expand "Build Documentation" step to see details
4. Check "Link checker" step for broken links
...
**Root Cause:**
The homodyne package is not installed in the RTD environment.
...
**Root Cause:**
JAX is not installed in RTD environment (may not be in docs/requirements.txt or pyproject.toml).
...
**Root Cause:**
Theme not listed in `docs/requirements.txt`.
...
2. **Verify `docs/conf.py` contains:**
   ```python
...
**Root Cause:**
Build takes too long, likely due to:
...
2. Disable image generation if not critical
3. Move large examples to separate pages

**Option B: Increase Timeout**
1. Go to RTD Admin → Advanced Settings
2. Set "Build timeout" to 1200 (20 minutes)
3. Only increase if build is genuinely useful

**Option C: Disable Expensive Features**
In `docs/conf.py`:
```python
...
**Symptom:**
Changes pushed to GitHub but RTD still shows old version.
...
2. **If Still Doesn't Update:**
   ...
   - Verify push was successful: `git log --oneline origin/main` (should show your commit)
   - Check RTD email for build notifications

3. **Recreate Webhook (Last Resort):**
...
**Symptom:**
Search box shows but returns no results.
...
**Symptom:**
Documentation displays in default theme, not RTD theme.
...
**Symptom:**
GitHub Actions reports broken links in documentation.
...
| Issue | Solution |
|-------|----------|
| External links offline | Mark as `ignore` in Sphinx config |
| Broken internal links | Fix file paths or anchor names |
| Links to GitHub that require auth | Mark as ignored (known issue) |
| Links that sometimes redirect | May be transient, recheck |
...
3. **Ignore external links (if needed):**
   In `docs/conf.py`:
   ```python
   linkcheck_ignore = [
...
**Symptom:**
PDF or ePub downloads not available on RTD.
...
**Symptom:**
Version dropdown shows but doesn't switch versions.
