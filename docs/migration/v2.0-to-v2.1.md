...
**Version**: 2.1.0 **Release Date**: October 31, 2025 **Migration Complexity**: Medium
(Breaking changes in CLI and YAML)

## Overview

Homodyne v2.1.0 simplifies the MCMC workflow by removing confusing CLI method flags
(`--method nuts`, `--method cmc`) and automatic NLSQ/SVI initialization. This results in
a **75% reduction in CLI complexity** while maintaining full functionality through
...
- ✅ Configurable thresholds in YAML: `min_samples_for_cmc: 15`,
...
- [ ] Add new thresholds to YAML: `min_samples_for_cmc: 15`,
  `memory_threshold_pct: 0.30`
...
1. **Update documentation** that mentions `nuts` or `cmc` methods

1. **Test** automatic selection works as expected for your datasets
...
1. **Add new threshold parameters** at `optimization.mcmc` level:
...
1. **Keep `initial_parameters`** structure unchanged:
...
**Method selection** | `method="nuts"` param | Automatic (removed param) | Remove param,
let system decide | | **Parameter passing** | `initial_params=dict` |
`initial_values=list` | Load from config instead | | **Bounds/priors** | Manual dict
construction | `ParameterSpace` object | Use `ParameterSpace.from_config()` | |
**Decision logic** | User responsibility | Dual-criteria OR logic | No code change
needed | | **Result format** | `(popt, samples, ...)` | Unified dict with metadata |
Update result parsing code |
...
1. **Create parameter extraction script** (optional):
...
1. **Update documentation** for your analysis pipeline

______________________________________________________________________

## Shell Aliases Migration

Shell aliases provide convenient shortcuts for common commands. In v2.1.0, we've removed
aliases for `nuts` and `cmc` methods since they no longer exist.
...
**Rationale**: Automatic selection ensures optimal performance and prevents user errors
(e.g., NUTS on 100 samples causing OOM)
...
**Symptom**:
`TypeError: fit_mcmc_jax() got unexpected keyword argument 'initial_params'`
...
**Symptom**: Decreased `memory_threshold_pct` from 0.30 to 0.20, but more datasets now
use CMC
...
1. Copy results to `initial_parameters.values` in YAML
1. Run MCMC
...
1. Add new threshold parameters (optional but recommended)
1. Update any scripts using `--method nuts` or `--method cmc`
...
1. Retry with different random seed (max 3 attempts)
1. If all fail, return result with `converged=False`
1. User should try manual NLSQ → MCMC workflow
...
**Can I restore old CLI flags for backward compatibility?** No. v2.1.0 does not support
deprecation warnings or compatibility layers for old flags. This is an acknowledged hard
breaking change. You must update your workflows.

**Will old configuration files still work?** Partially. You must:

1. Remove `mcmc.initialization` section
1. Add new parameters (optional but recommended)
1. Update any shell scripts using old method flags
...
- Large datasets (> 50 samples, high memory): NUTS → CMC (v2.1 parallelized) = ~2-3x
  speedup
...
**Support:** See [CLAUDE.md](../../CLAUDE.md) for detailed documentation or open an
issue on GitHub.
