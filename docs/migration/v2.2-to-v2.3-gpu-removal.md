...
**Breaking Change Type:** Hard break - No migration path provided **Last GPU-Supporting
Version:** v2.2.1 **Rationale:** Simplify maintenance, focus on reliable CPU execution

______________________________________________________________________

## Executive Summary

Homodyne v2.3.0 removes all GPU acceleration support in favor of a CPU-only
architecture. This is a **hard breaking change** with no migration path. Users who
require GPU acceleration should remain on **v2.2.1**, the last GPU-supporting version.
...
**Note:** v2.3.0 gracefully ignores legacy GPU keys for backward compatibility with old
config files.
...
`configure_system_cuda()` | CUDA initialization | Use `configure_optimal_device()`
(CPU-only) | | `detect_system_cuda()` | CUDA detection | Not applicable | |
...
`is_gpu_active()` | GPU status check | Always returns False | | `switch_to_cpu()` |
Force CPU mode | Not applicable (always CPU) |
...
`make gpu-check` | Validate GPU/CUDA setup | Not applicable | | `make test-gpu` | Run
GPU-specific tests | Not applicable |
...
`examples/gpu_accelerated_optimization.py` | GPU optimization demo | Use
`examples/cpu_optimization.py` | | `examples/gpu_acceleration.py` | GPU features
showcase | Use `examples/multi_core_batch_processing.py` |
...
| GPU sections in installation guide | Removed | CPU-only installation | | GPU
performance benchmarks | Removed | CPU performance guides |
...
~5s | ~6s | CPU competitive | | Medium dataset (1M points) | ~30s | ~45s | CPU 1.5x
slower, but reliable | | Large dataset (3M points) | GPU OOM | ~150s | **CPU succeeds
where GPU failed** | | Per-angle scaling (3M points) | GPU OOM | ~150s | **CPU only
reliable solution** | | MCMC (20 samples) | ~40 min | ~40 min | CPU parallelization
competitive |

**Key Takeaway:** CPU-only v2.3.0 provides **reliable, predictable performance** without
GPU OOM errors.
...
1. **Reliability** - GPU OOM errors hard to debug, CPU more predictable
1. **Performance** - Multi-core CPU provides excellent performance for XPCS
...
1. Stay on v2.2.1 with GPU support
1. Use data subsampling for exploratory analysis
...
**A:** No. Results from v2.2.1 GPU and v2.3.0 CPU should be numerically identical
...
1. Verify configuration files (remove GPU keys)
1. Report issues on GitHub with "v2.3.0 migration" label
1. If blocked: downgrade to v2.2.1 temporarily
...
**Solution:** Remove GPU function imports:
...
**Solution:** Warnings are harmless (graceful degradation), but clean up config:
...
**Symptom:** Slower execution time compared to v2.2.1 GPU
...
**Symptom:** CPU-only performance insufficient for your use case

**Solution:** Stay on v2.2.1:
...
| Date | Event | Details | |------|-------|---------| | **November 6, 2025** | v2.2.1
released | Last GPU-supporting version | | **November 7, 2025** | v2.3.0 released | GPU
support removed | | **November 7, 2025 - May 2026** | v2.2.1 critical bug fixes | Best
effort maintenance | | **May 2026+** | v2.2.1 maintenance mode | Available but
unsupported |
