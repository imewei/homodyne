...
1. **Deprecated XLA Flag Removal**: Cleaned up deprecated `--xla_cpu_use_thunk_runtime`
   flag
1. **Intel oneDNN Support**: Optional Intel oneDNN optimizations with ~15% performance
   improvement
1. **Benchmark Tool**: New `benchmark_onednn.py` for testing oneDNN on your system
...
**Background**: The `--xla_cpu_use_thunk_runtime` flag was deprecated in JAX and will be
removed in future releases. The thunk-based CPU runtime has been the default since JAX
0.4.32.
...
**Date**: 2025-11-09 **Status**: ✅ Production Ready
...
|----------------|------------------|----------------| | **Intel 12th-14th Gen**
(Alder/Raptor Lake) | 1.10-1.20x (10-20%) | ✅ Enable | | **Intel 10th-11th Gen**
(Ice/Tiger Lake) | 1.05-1.15x (5-15%) | ⚠️ Benchmark first | | **Intel 8th-9th Gen**
(Coffee Lake) | 1.00-1.10x (0-10%) | ⚠️ Benchmark first | | **Intel 7th Gen or older** |
1.00-1.05x (0-5%) | ❌ Likely no benefit | | **AMD CPUs** | Not supported | ❌
Auto-skipped |
...
1. **Memory Layout**: Better cache utilization for multi-dimensional arrays
1. **Vectorization**: Enhanced AVX2 vectorization for Intel microarchitecture
1. **Compiler Optimizations**: Additional XLA compiler passes enabled
...
1. **AVX2 recommended**: Older CPUs without AVX2 may see minimal benefit
1. **Measurement variability**: Benchmark results may vary ±5% between runs
1. **First-run overhead**: JIT compilation time not included in benchmarks

## Future Work

1. **Extended benchmarking**: Test on more CPU generations (8th-11th Gen Intel)
1. **Real dataset validation**: Compare with synthetic benchmarks
1. **Numerical consistency**: Verify results match reference implementation
1. **AMD alternative**: Investigate BLAS library optimizations for AMD CPUs
...
**Last Updated**: 2025-11-09 **Homodyne Version**: 2.3.0+ **JAX Version**: 0.8.0
(CPU-only)
