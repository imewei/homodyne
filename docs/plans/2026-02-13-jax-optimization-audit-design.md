...
**Date**: 2026-02-13 **Scope**: Safe, decorator/wrapper-level optimizations only (no API
or logic changes) **Target workload**: 8-200M points (50M typical), 10-30 phi angles,
CPU-only
...
- `core/physics_nlsq.py` — meshgrid functions (n_phi, n_points are compile-time
  constants)
...
**Impact**: 50-80% reduction in recompilation overhead. **Risk**: Zero.
...
**Impact**: 3-5x speedup for scaling estimation on 20+ angle datasets. **Risk**: Low.
Data-prep step, not JIT hot path. Falls back to loop if needed.
...
**Impact**: Eliminates Python-level JIT wrapper overhead on repeated calls. **Risk**:
Near-zero. Standard memoization.
...
**Impact**: Varies by hardware. AVX-512 exploitation can yield 2x throughput. **Risk**:
Zero. Environment variables set before computation.

## Success Criteria

1. No API changes — all function signatures preserved
1. Existing tests pass without modification
1. JIT recompilation count measurably reduced (Section 1)
1. Per-angle scaling computation measurably faster for n_phi >= 10 (Section 2)
