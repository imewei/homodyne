...
Homodyne v2.2 introduces **angle-stratified chunking**, a critical fix for per-angle scaling parameter optimization on large datasets (>1M points). This release resolves silent NLSQ optimization failures caused by incompatibility between per-angle parameters and arbitrary data chunking.
...
2. **NLSQ chunking** splits large datasets into arbitrary chunks (e.g., 3M → 11×300k)
3. **Problem**: Chunks may not contain all phi angles
4. **Result**: Gradient w.r.t. missing angle parameters = 0 → optimization failure
...
- **Workarounds (pre-v2.2):**
  ...
  2. Reduce dataset size via phi filtering (<1M points)
  3. Force STANDARD strategy (no chunking, memory intensive)
  4. Use MCMC instead of NLSQ

## Solution: Angle-Stratified Chunking

### Overview

Reorganize data **before** NLSQ optimization to ensure every chunk contains all phi angles:
...
2. **Transparent** - Existing workflows unchanged
3. **Configurable** - Full control when needed
4. **Efficient** - <1% overhead, 2x memory peak (temporary)
5. **Safe** - Memory safety checks built-in
...
2. **Integration Tests** (`tests/integration/test_stratified_nlsq_integration.py`): 12/12 ✅
   - Configuration parsing (4 tests)
   - Strategy selection (3 tests)
   - Data stratification (3 tests)
   - Component integration (2 tests)

3. **Performance Benchmarks** (`tests/performance/test_stratified_chunking_performance.py`): 12/12 ✅
...
2. Stashed stratification changes and re-ran failing tests
3. Confirmed identical failures without our modifications
4. **Conclusion:** All observed failures are pre-existing issues
...
2. **Sequential per-angle fallback** not yet implemented
   - Use `force_sequential_fallback: true` for placeholder
   - Will be implemented in Phase 3

3. **Stratification only for per-angle scaling**
...
**Overview:**
Fallback optimization strategy when angle-stratified chunking cannot be used.
Optimizes each phi angle independently using scipy.optimize.least_squares,
then combines results using inverse variance weighting.
...
- Coverage: Data splitting, single angle optimization, result combination, end-to-end, error handling
...
**Overview:**
Zero-copy stratification using index arrays instead of full data copies.
...
- Coverage: Index length, no duplicates, data preservation, equivalence to full copy, memory efficiency
...
**Overview:**
Comprehensive diagnostics for stratification performance, chunk balance, and angle coverage.
Provides detailed metrics with formatted reporting for troubleshooting and validation.
...
- Coverage: Dataclass construction, diagnostics computation (balanced/imbalanced/large datasets), report formatting, end-to-end workflows
...
- Ultra-Think Analysis: `/home/wei/Documents/Projects/data/C020/homodyne_results/logs/ultra-think-20251106-012247.md`
...
- `homodyne/optimization/sequential_angle.py` - Sequential per-angle optimization (491 lines)
...
- `homodyne/optimization/nlsq_wrapper.py` - OptimizationResult.stratification_diagnostics field

**Configuration:**
- `homodyne/config/types.py` - StratificationConfig + SequentialConfig (+144 lines total)
- `homodyne/config/templates/homodyne_static.yaml` - Template sections (+52 lines total)

**Tests (v2.2.0):**
- `tests/unit/test_stratified_chunking.py` - Stratification unit tests (480 lines, 23 tests)
- `tests/integration/test_stratified_nlsq_integration.py` - Integration (302 lines, 12 tests)
- `tests/performance/test_stratified_chunking_performance.py` - Benchmarks (485 lines, 12 tests)

**Tests (v2.2.1):**
- `tests/unit/test_sequential_angle.py` - Sequential optimization tests (600 lines, 18 tests)
