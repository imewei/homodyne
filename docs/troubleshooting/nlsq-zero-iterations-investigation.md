...
**Status**: Unresolved - Deep architectural incompatibility identified
**Symptom**: NLSQ optimization consistently returns 0 iterations with unchanged parameters
**Impact**: Laminar flow analysis with >1M points cannot be optimized
**Root Cause**: JAX JIT compilation incompatibility with model function architecture
...
The model function architecture is **fundamentally incompatible** with NLSQ's JIT compilation:
...
1. **Model returns constant**: Full array return means model output doesn't vary correctly with chunk requests
2. **Gradient computation fails**: JAX can't compute gradients when model/data size mismatch
3. **JIT compilation artifact**: Traced computations produce degenerate gradients
...
2. **Dynamic indexing forbidden**: `array[tracer]` → TracerArrayConversionError
3. **Chunk identification impossible**: Can't determine which chunk without concrete indices
4. **Full array breaks gradients**: Returning full array when chunk requested = gradient mismatch
...
**Pros**: Avoids tracer issues entirely
**Cons**: Orders of magnitude slower, defeats purpose of JAX
...
**Pros**: Truly chunk-compatible, efficient
**Cons**: Requires complete rewrite of data pipeline
...
**Pros**: Avoids JAX JIT issues entirely
**Cons**: Loses GPU acceleration, may be slower
...
**Pros**: Full control over gradient computation
...
2. **Add warning** when laminar_flow + large dataset detected
3. **Suggest phi angle filtering** to reduce dataset size below 1M points

### Medium-term (1-2 weeks)
1. **Test Option 3**: Evaluate scipy.optimize.least_squares performance
2. **Benchmark**: Compare scipy vs NLSQ (with jit=False) vs current state
3. **Prototype**: Chunk-aware architecture proof-of-concept

### Long-term (1-2 months)
1. **Implement Option 2**: Full chunk-aware model restructure
2. **Add tests**: Integration tests for large datasets
3. **Performance**: Profile and optimize new architecture
...
- **Original Bug**: `/home/wei/Documents/Projects/data/C020/homodyne_results/logs/homodyne_analysis_20251104_173024.log`
- **Investigation Logs**: `homodyne_analysis_20251105_*.log`
- **NLSQ Docs**: https://nlsq.readthedocs.io/en/latest/
- **JAX Tracer Error**: https://jax.readthedocs.io/en/latest/errors.html#jax.errors.TracerArrayConversionError
...
2. NLSQ's JIT-compiled chunking expectations
3. JAX's tracer-based JIT compilation
...
Until resolved, users must work around by reducing dataset sizes or accepting unoptimized results.

---

**Last Updated**: 2025-11-05
**Investigated By**: Claude Code Debugging Session
