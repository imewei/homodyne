...
from homodyne.utils.logging import configure_logging, get_logger
...
def run_analysis(
    ...
    **kwargs,
    ...
    if output_dir is None:
        output_dir = (
            Path.cwd() / f"homodyne_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    try:
        ...
        executor = MethodExecutor(
            ...
            disable_dataset_optimization=kwargs.get(
                "disable_dataset_optimization", False
            ),
        )

        # Execute analysis
        result = _execute_analysis_method(executor, method, data_dict, config)
...
def load_and_analyze(
    data_file: Union[str, Path], methods: List[str] = ["vi", "mcmc"], **kwargs
...
def compare_methods(
    data_file: Union[str, Path], output_dir: Optional[Union[str, Path]] = None, **kwargs
) -> Dict[str, Any]:
    """
    ...
    results = load_and_analyze(
        data_file, methods=methods, output_dir=output_dir, **kwargs
    )
...
def run_complete_pipeline(
    ...
    **kwargs,
    ...
    if output_dir is None:
        output_dir = (
            Path.cwd() / f"homodyne_pipeline_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    args.disable_dataset_optimization = kwargs.get(
        "disable_dataset_optimization", False
    )
    ...
    try:
        ...
        if pipeline.config:
            pipeline_results["analysis_mode"] = pipeline.config.get(
                "analysis_mode", "unknown"
            )
            pipeline_results["effective_parameter_count"] = pipeline.config.get(
        ...
        if exit_code == 0:
            ...
            logger.info(
                f"   Analysis mode: {pipeline_results.get('analysis_mode', 'unknown')}"
            )
        else:
...
class AnalysisSession:
    ...
    def __init__(
        ...
        self.output_dir = (
            Path(output_dir) if output_dir else Path.cwd() / "session_results"
        )
    ...
    def load_data(self, data_file: Union[str, Path]) -> None:
        ...
        logger.info(
            f"  Time range: t1=[{self.data_dict['t1'][0]:.2e}, {self.data_dict['t1'][-1]:.2e}]"
        )
    ...
    def analyze(self, method: str, use_pipeline: bool = False, **kwargs) -> ResultType:
        ...
        if use_pipeline:
            ...
            if not pipeline_result["success"]:
                raise RuntimeError(
                    f"Pipeline {method.upper()} analysis failed: {pipeline_result.get('error', 'Unknown error')}"
                )
            ...
            # Execute analysis
            executor = MethodExecutor(
                config=session_config, force_cpu=kwargs.get("force_cpu", False)
            )

            result = _execute_analysis_method(
                executor, method, self.data_dict, session_config
            )
    ...
    def export_results(self, formats: List[str] = ["yaml", "npz"]) -> Dict[str, Path]:
        ...
        for method, result in self.results.items():
            ...
            method_paths = results_manager.export_results_multiple_formats(
                result, formats
            )
    ...
    **kwargs,
    ...
    config = {
        ...
        "optimization": {
            ...
            "hybrid": {"use_vi_init": kwargs.get("use_vi_init", True)},
        },
        "hardware": {
            "force_cpu": kwargs.get("force_cpu", False),
            "gpu_memory_fraction": kwargs.get("gpu_memory_fraction", 0.8),
...
def _create_result_summary(
    ...
    summary = {
        ...
        "parameters": (
            result.mean_params.tolist() if hasattr(result, "mean_params") else None
        ),
        "chi_squared": getattr(result, "chi_squared", None),
        "output_directory": str(output_dir),
        "data_summary": {
