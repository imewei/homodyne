...
# Suppress verbose logging from performance components during initialization
import logging

logging.getLogger("homodyne.config.performance_cache").setLevel(logging.WARNING)
logging.getLogger("homodyne.config.memory_optimizer").setLevel(logging.WARNING)
logging.getLogger("homodyne.config.performance_profiler").setLevel(logging.WARNING)

from .lazy_validator import (LazyValidator, ValidationLevel,
                             ValidationPriority, ValidationTask,
                             get_lazy_validator, set_global_validation_level)
# Import order optimized for performance and dependency resolution
from .manager import ConfigManager, PerformanceMonitor, configure_logging
from .memory_optimizer import MemoryMonitor
from .memory_optimizer import StreamingConfig as MemStreamingConfig
from .memory_optimizer import (StreamingProcessor, get_memory_monitor,
                               get_streaming_processor,
                               memory_efficient_processing)
from .mode_resolver import ModeResolver
from .modes import detect_analysis_mode
from .optimized_mode_resolver import (AnalysisMetrics, OptimizedModeResolver,
                                      StreamingConfig)
from .parallel_validator import (ParallelValidator, ValidationJob,
                                 WorkerConfig, get_parallel_validator,
                                 validate_configurations_parallel)
# Legacy compatibility imports
from .parameter_validator import ParameterValidator, ValidationResult
# Performance optimization imports
from .performance_cache import (PerformanceCache, ValidationResultCache,
                                clear_all_caches, get_performance_cache,
...
def get_optimization_status():
    ...
    try:
        ...
        optional_features["numba_acceleration"] = {
            "available": True,
        ...
        optional_features["numba_acceleration"] = {
            "available": False,
            "impact": "JIT compilation disabled",
        }
    ...
    try:
        ...
        optional_features["line_profiling"] = {
            "available": False,
            "impact": "Line-by-line profiling disabled",
        }
    ...
    try:
        ...
        optional_features["memory_profiling"] = {
            "available": False,
            "impact": "Advanced memory profiling disabled",
        }
...
def _get_optimization_recommendations(system_info, optional_features):
    ...
    if cpu_count >= 8:
        recommendations.append(
            "Your system has many cores - consider increasing parallel worker counts"
        )

    if memory_gb >= 16:
        recommendations.append(
            "High memory system - can enable larger cache sizes and batch processing"
        )
    elif memory_gb < 4:
        recommendations.append(
            "Limited memory - enable streaming processing and reduce cache sizes"
        )
    ...
    if not optional_features.get("line_profiling", {}).get("available", False):
        recommendations.append(
            "Install line_profiler for detailed performance analysis"
        )

    if not optional_features.get("memory_profiling", {}).get("available", False):
        recommendations.append(
            "Install memory_profiler for advanced memory optimization"
        )
...
def print_optimization_status():
    ...
    for feature_name, feature_info in status["optional_features"].items():
        ...
        print(
            f"  {status_indicator} {feature_name:25}: {'Available' if feature_info['available'] else 'Not Available'}"
        )
