...
class HPCValidator:
        ...
        if detected_scheduler in hpc_config:
            ...
            if detected_scheduler == "pbs":
                ...
                result.warnings.append(
                    f"Scheduler '{detected_scheduler}' detected but not fully supported"
                )
        else:
            result.warnings.append(
                f"Scheduler '{detected_scheduler}' detected but not configured"
            )
        ...
        for scheduler, commands in self.scheduler_detection_commands.items():
            for command in commands:
                if shutil.which(command):
                    logger.debug(
                        f"Detected {scheduler.upper()} scheduler (command: {command})"
                    )
                    return scheduler
        return None

    def _validate_pbs_configuration(
        self, pbs_config: Dict[str, Any], result: HPCValidationResult
    ) -> None:
        """
    ...
    def _validate_slurm_configuration(
        self, slurm_config: Dict[str, Any], result: HPCValidationResult
    ) -> None:
        """
        ...
        if partition == "gpu" and "gres" not in slurm_config:
            result.warnings.append(
                "GPU partition selected but no GPU resources (gres) specified"
            )

        result.estimated_walltime = time_limit

    def _validate_distributed_settings(
        self, dist_config: Dict[str, Any], result: HPCValidationResult
    ) -> None:
        """
        Validate distributed computing settings.
        ...
        if "mpi" in dist_config:
            ...
            if not self._check_mpi_availability():
                result.warnings.append(
                    "MPI configuration specified but MPI not available"
                )
                result.is_valid = False
            else:
                result.recommendations.append(
                    "MPI detected - ensure proper module loading in job script"
                )
    ...
    def _calculate_resource_efficiency(
        self, hpc_config: Dict[str, Any], result: HPCValidationResult
    ) -> None:
        """
    ...
    def _convert_to_gb(self, value: float, unit: str) -> float:
        ...
        conversions = {"b": 1e-9, "kb": 1e-6, "mb": 1e-3, "gb": 1, "tb": 1e3}
...
class GPUValidator:
    ...
    def validate_gpu_configuration(
        self, hardware_config: Dict[str, Any], optimization_config: Dict[str, Any]
    ) -> GPUValidationResult:
        """
    ...
    def _validate_gpu_memory_config(
        self,
    ...
    def _validate_gpu_software_compatibility(
        self, optimization_config: Dict[str, Any], result: GPUValidationResult
    ) -> None:
        """
        ...
        if "gpu_backend" in mcmc_config.get("backend_specific", {}):
            if not HAS_JAX:
                ...
                try:
                    ...
                    result.warnings.append("JAX GPU backend initialization failed")
    ...
    def _generate_gpu_optimization_suggestions(
        self,
        hardware_config: Dict[str, Any],
        gpu_info: Dict[str, Any],
        result: GPUValidationResult,
...
class AdvancedScenarioValidator:
    ...
    def validate_large_dataset_scenario(
        self, config: Dict[str, Any], estimated_data_size: int
    ) -> Dict[str, Any]:
        """
    ...
    def validate_batch_processing_scenario(
        ...
        try:
            ...
            if not os.access(output_path, os.W_OK):
                result["warnings"].append(
                    f"Output directory not writable: {output_directory}"
                )
                result["is_valid"] = False
        except Exception as e:
    ...
    def validate_complex_phi_filtering(
        self, angle_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _estimate_runtime(
        self, data_size: int, mode: str, config: Dict[str, Any]
    ) -> float:
        """
    ...
    def _add_config_specific_recommendations(
        self, config: Dict[str, Any], data_size: int, result: Dict[str, Any]
    ) -> None:
        """
