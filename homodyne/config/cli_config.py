...
class CLIConfigManager:
    ...
    def create_effective_config(
        self, config_file: Optional[str], args: argparse.Namespace
    ) -> Dict[str, Any]:
        """
    ...
    def _extract_cli_overrides(self, args: argparse.Namespace) -> Dict[str, Any]:
        ...
        if (
            hasattr(args, "disable_dataset_optimization")
            and args.disable_dataset_optimization
        ):
    ...
    def _extract_optimization_overrides(
    ...
    def _merge_configurations(
        self, base_config: Dict[str, Any], cli_overrides: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _resolve_analysis_mode(self, args: argparse.Namespace) -> None:
        ...
        resolved_mode = resolver.resolve_mode(
            config=self.effective_config, cli_args=args
        )

        if resolved_mode != self.effective_config.get("analysis_mode"):
            logger.info(
                f"Analysis mode resolved: {self.effective_config.get('analysis_mode')} → {resolved_mode}"
            )
