...
class ConfigMerger:
    ...
    def merge_configurations(
        ...
        result = MergeResult(config={}, conflicts=[], sources={}, warnings=[])
        ...
        for key, source_value in source.items():
            ...
            if isinstance(target_value, dict) and isinstance(source_value, dict):
                ...
                if key in self.special_merge_keys:
                    merged_value = self.special_merge_keys[key](
    ...
    def _merge_hardware_config(
        ...
        if "gpu_memory_fraction" in source:
            ...
            if 0.0 < gpu_fraction <= 1.0:
                ...
                logger.warning(
                    f"Invalid gpu_memory_fraction {gpu_fraction}, keeping {target.get('gpu_memory_fraction', 0.8)}"
                )
    ...
    def _ensure_mode_consistency(
        self, config: Dict[str, Any], result: MergeResult
    ) -> None:
        """
    ...
    def _get_base_defaults(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "data": {"dataset_optimization": True},
            "optimization": {
                "vi": {
                    "n_iterations": 2000,
                    "learning_rate": 0.01,
            ...
            "output": {"formats": ["yaml", "npz"], "include_diagnostics": True},
        }
            ...
            if key not in config1:
                ...
                if isinstance(config1[key], dict) and isinstance(config2[key], dict):
                    self._find_differences(
                        config1[key], config2[key], differences, current_path
                    )
                else:
