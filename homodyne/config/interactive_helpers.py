...
class InteractiveConfigurationBuilder:
    ...
    def ask_value(self, prompt: str, value_type: type = str, default: Any = None, validator: Optional[Callable] = None) -> Any:
    ...
    def run_interactive_setup(self) -> Dict[str, Any]:
        ...
        print(self.colorize("\n🎯 Welcome to the Homodyne v2 Interactive Configuration Builder!", 'bold'))
        print("This tool will guide you through creating a complete configuration for your XPCS analysis.\n")
    ...
    def _setup_analysis_mode(self):
        ...
        modes = [
            ("static_isotropic", "Static system, no directional dependence", "Fastest, simplest"),
            ("static_anisotropic", "Static system with directional effects", "Medium complexity"),
        ...
        for i, (mode, desc, performance) in enumerate(modes, 1):
            ...
            print(f"     {desc}")
            print(f"     Performance: {self.colorize(performance, 'green' if 'Fast' in performance else 'yellow')}")
            print()
        
        # Provide guidance based on user level
        if self.user_level == "novice":
            print("💡 Recommendation: Start with 'Static Isotropic' for your first analysis")
            default_choice = 1
        else:
            default_choice = 2
        
        choice_num = int(input(f"Choice [1-3, default: {default_choice}]: ").strip() or str(default_choice))
    ...
    def _setup_experimental_parameters(self):
        ...
        if self.user_level == "novice":
            print("These parameters define the time window and resolution of your analysis.")
        ...
        end_frame = self.ask_value(
            ...
            int,
            default=1000,
            validator=lambda x: x > start_frame
        )
    ...
    def _setup_data_configuration(self):
        ...
        data_file = self.ask_value(
            "Data file name (HDF5 format)",
            str,
            default="your_data.hdf"
        )
        ...
        if self.user_level != "novice":
            use_cache = self.ask_yes_no(
                "Enable data caching for faster repeated analysis?", 
                default=True
            )
            if use_cache:
        ...
        if additional_settings:
            print(f"   • Caching enabled: {additional_settings.get('cache_file_path', 'N/A')}")
    ...
    def _setup_analysis_settings(self):
        ...
        if self.user_level != "novice":
            ...
            if customize:
                ...
                custom_values = []
                for i, (name, default_val) in enumerate(zip(param_names, default_values)):
        ...
        if self.analysis_mode == "static_isotropic":
            ...
            model_desc = "g₁(t₁,t₂) = g₁_diff(t₁,t₂) × g₁_shear(t₁,t₂) with shear effects"
    ...
    def _setup_optimization_settings(self):
        ...
        if self.analysis_mode == "static_isotropic":
            ...
            print("Angle filtering improves analysis by focusing on specific angular ranges.")
        ...
        if angle_filtering_enabled:
            if self.user_level == "expert":
                ...
                while add_range:
                    ...
                    min_angle = self.ask_value("  Minimum angle", float, default=-10.0 if range_num == 1 else 170.0)
                    max_angle = self.ask_value("  Maximum angle", float, default=10.0 if range_num == 1 else 190.0)
                    ...
                    if min_angle < max_angle:
                        ...
                        if range_num <= 2:
                            add_range = self.ask_yes_no("Add another angle range?", default=range_num == 2)
                        else:
                            add_range = self.ask_yes_no("Add another angle range?", default=False)
                    else:
        ...
        if self.user_level == "expert":
            ...
            include_classical = self.ask_yes_no(
                "Include classical optimization methods?",
                default=True
            )
            ...
            if include_classical:
                ...
                if selected_methods:
                    self.config_data["optimization_config"]["classical_optimization"] = {
                        "methods": selected_methods,
                        "method_options": {
                            "Nelder-Mead": {"maxiter": 1000, "xatol": 1e-8, "fatol": 1e-8},
                            "Powell": {"maxiter": 1000, "xtol": 1e-8, "ftol": 1e-8},
                            "L-BFGS-B": {"maxiter": 15000, "ftol": 2.220446049250313e-09}
                        }
                    }
    ...
    def _setup_logging_configuration(self):
        ...
        print(f"   • File: {log_to_file}" + (f" ({logging_config.get('log_filename', 'N/A')})" if log_to_file else ""))
    ...
    def _review_and_save(self):
        ...
        print(f"   • Parameters: {len(self.config_data['initial_parameters']['parameter_names'])}")
        ...
        if "optimization_config" in self.config_data:
            angle_enabled = self.config_data["optimization_config"]["angle_filtering"]["enabled"]
...
class ConfigurationValidator:
    ...
    def validate_configuration(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        if missing_sections:
            ...
            result.suggestions.append("Add missing sections using the interactive builder")
        ...
        if result.score >= 80:
            result.performance_notes.append(f"Excellent configuration (score: {result.score}/100)")
        elif result.score >= 60:
            result.performance_notes.append(f"Good configuration (score: {result.score}/100)")
            result.suggestions.append("Consider addressing warnings to improve score")
        else:
            result.performance_notes.append(f"Configuration needs improvement (score: {result.score}/100)")
            result.suggestions.append("Use interactive builder to fix issues")
        
        return result
    
    def _validate_parameter_ranges(self, config: Dict[str, Any], result: ValidationResult) -> None:
        ...
        for param, value in temporal.items():
            if param in self.validation_rules["parameter_ranges"]:
                ...
                if not (min_val <= value <= max_val):
                    result.warnings.append(f"{param} = {value} outside typical range [{min_val}, {max_val}]")
                    result.suggestions.append(f"Consider adjusting {param} to be within typical range")
        ...
        if q is not None:
            ...
            if not (min_q <= q <= max_q):
                result.warnings.append(f"wavevector_q = {q} outside typical XPCS range [{min_q}, {max_q}]")
                
            if q > 0.1:
                result.performance_notes.append("Large q-vector may require longer computation time")
        
        # Check frame range consistency
        start = temporal.get("start_frame", 1)
        end = temporal.get("end_frame", 100)
        if start >= end:
            result.errors.append(f"Invalid frame range: start_frame ({start}) >= end_frame ({end})")
            result.is_valid = False
        
        frame_count = end - start
        if frame_count < 10:
            result.warnings.append(f"Very few frames ({frame_count}) may give poor statistics")
        elif frame_count > 10000:
            result.performance_notes.append(f"Large frame count ({frame_count}) will increase computation time")
    
    def _validate_mode_consistency(self, config: Dict[str, Any], result: ValidationResult) -> int:
        ...
        expected_count = self.validation_rules["mode_parameter_counts"].get(expected_mode, 3)
        ...
        if param_count != expected_count:
            ...
            result.performance_notes.append(f"Parameter count correctly matches {expected_mode} mode")
            score_delta += 5
        ...
        if expected_mode == "static_isotropic" and filtering_enabled:
            result.warnings.append("Angle filtering enabled for isotropic mode (will be ignored)")
            result.suggestions.append("Disable angle filtering for isotropic mode to avoid confusion")
            score_delta -= 5
        elif expected_mode != "static_isotropic" and not filtering_enabled:
            result.suggestions.append("Consider enabling angle filtering for better analysis quality")
        
        return score_delta
    
    def _validate_performance_settings(self, config: Dict[str, Any], result: ValidationResult) -> int:
        ...
        if v2_features:
            ...
            if v2_features.get("parallel_processing", False):
                result.performance_notes.append("Parallel processing enabled - may speed up analysis")
                score_delta += 5
            
            if v2_features.get("gpu_acceleration", False):
                result.performance_notes.append("GPU acceleration enabled - significant speedup expected")
                score_delta += 10
            
            cache_strategy = v2_features.get("cache_strategy", "intelligent")
            if cache_strategy == "disabled":
                result.performance_notes.append("Caching disabled - analysis will be slower")
                score_delta -= 5
            elif cache_strategy == "intelligent":
                result.performance_notes.append("Intelligent caching enabled - good balance")
                score_delta += 3
        
        return score_delta
    
    def _validate_data_accessibility(self, config: Dict[str, Any], result: ValidationResult) -> None:
        ...
        if data_folder and data_file:
            ...
            if not Path(data_folder).exists():
                ...
                result.suggestions.append("Create data folder or update path before running analysis")
            elif not data_path.exists():
                result.warnings.append(f"Data file not found: {data_path}")
                result.suggestions.append("Place your data file in the specified location")
            else:
    ...
    def _validate_logging_config(self, config: Dict[str, Any], result: ValidationResult) -> None:
        ...
        if not logging_config.get("log_to_console", True) and not logging_config.get("log_to_file", False):
            result.warnings.append("Both console and file logging are disabled - no output will be visible")
            result.suggestions.append("Enable at least console logging to see analysis progress")
    ...
    def validate_realtime(self, config_section: str, config_value: Any) -> List[str]:
        ...
        if config_section == "wavevector_q" and isinstance(config_value, (int, float)):
            if config_value <= 0:
                ...
                issues.append("Very large wavevector - typical XPCS range is 0.001-0.1 Å⁻¹")
            ...
            if start >= end:
                ...
                issues.append("Very few frames - consider using more frames for better statistics")
...
class ConfigurationComparator:
    ...
    def _estimate_performance(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        angle_filtering = config.get("optimization_config", {}).get("angle_filtering", {}).get("enabled", True)
...
def create_interactive_builder(output_path: Optional[Union[str, Path]] = None) -> InteractiveConfigurationBuilder:
...
def compare_configuration_files(config1_path: Union[str, Path], 
