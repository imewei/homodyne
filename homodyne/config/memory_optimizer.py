...
class MemoryMonitor:
    ...
    def __init__(self, alert_threshold: float = 0.8, critical_threshold: float = 0.9):
        ...
        logger.debug(
            f"Memory monitor initialized: alert={alert_threshold:.1%}, critical={critical_threshold:.1%}"
        )
    ...
    def optimize_memory(self) -> Dict[str, Any]:
        ...
        optimization_stats["memory_freed_gb"] = max(
            0, initial_stats.process_memory_gb - final_stats.process_memory_gb
        )

        logger.info(
            f"Memory optimization completed: freed {optimization_stats['memory_freed_gb']:.2f}GB"
        )
        ...
        while self._monitoring:
            try:
                ...
                if stats.memory_utilization >= self.critical_threshold:
                    logger.critical(
                        f"CRITICAL: Memory usage {stats.memory_utilization:.1%} "
                    ...
                    logger.warning(
                        f"HIGH: Memory usage {stats.memory_utilization:.1%} "
                        f"(process: {stats.process_memory_gb:.2f}GB)"
                    )
                    self._trigger_alerts(stats)
...
class StreamingProcessor:
    ...
    def __init__(self, streaming_config: Optional[StreamingConfig] = None):
        ...
        if self.config.temp_directory is None:
            self.config.temp_directory = (
                Path(tempfile.gettempdir()) / "homodyne_streaming"
            )

        self.config.temp_directory.mkdir(parents=True, exist_ok=True)

        logger.info(
            f"Streaming processor initialized: chunk_size={self.config.chunk_size}, "
    ...
    def stream_large_file(
        ...
        logger.info(
            f"Streaming large file: {file_path} ({file_path.stat().st_size / (1024**2):.1f}MB)"
        )
        ...
        with self.memory_managed_processing():
            if (
                ...
                yield from self._stream_with_mmap(
                    file_path, parser_func, processor_func
                )
            else:
                yield from self._stream_with_buffered_read(
                    file_path, parser_func, processor_func
                )

    def create_memory_efficient_cache(
        self, cache_data: Dict[str, Any], cache_name: str = "config_cache"
    ) -> Path:
        """
        ...
        """
        cache_file = (
            self.config.temp_directory / f"{cache_name}_{int(time.time())}.cache"
        )
    ...
    def _auto_configure_streaming(self) -> StreamingConfig:
        ...
        config = StreamingConfig(
            ...
            use_memory_mapping=available_memory_gb
            > 4.0,  # Only if we have decent memory
            enable_compression=True,
        )

        logger.info(
            f"Auto-configured streaming: chunk_size={chunk_size}, "
    ...
    def _stream_with_buffered_read(
        self,
        file_path: Path,
        parser_func: Callable[[str], Dict[str, Any]],
        processor_func: Callable[[List[Dict[str, Any]]], List[Any]],
    ...
    def _adaptive_resize(self, memory_pressure: float) -> None:
        ...
        if memory_pressure > 0.9:
            ...
            self._current_chunk_size = min(
                self.config.chunk_size * 2, int(self._current_chunk_size * 1.2)
            )

        if self._current_chunk_size != old_size:
            logger.info(
                f"Adaptive chunk resize: {old_size} → {self._current_chunk_size} "
    ...
    def _spill_to_disk(self) -> None:
        ...
        if self._memory_pool:
            spill_file = (
                self.config.temp_directory
                / f"spill_{int(time.time())}_{threading.get_ident()}.tmp"
            )
    ...
    def _cleanup_temp_files(self) -> None:
        ...
        if cleaned_count > 0:
            logger.info(
                f"Cleaned up {cleaned_count} temp files ({cleaned_size / (1024**2):.1f}MB)"
            )
