...
class ModeResolver:
    ...
    def resolve_mode(
        ...
        for method in self.resolution_priority:
            try:
                ...
                if result and result != "auto-detect":
                    ...
                    return self._validate_and_return_enhanced(
                        result, config, data_dict, resolution_results
                    )
        ...
        logger.warning(
            f"All resolution methods failed, using fallback: {fallback_mode}"
        )
        self._log_resolution_summary("fallback", fallback_mode, resolution_results)
        return fallback_mode

    def _resolve_from_cli(
        self, cli_args: Optional[argparse.Namespace]
    ) -> Optional[str]:
        """
    ...
    def _analyze_phi_angles(self, phi_angles: np.ndarray) -> Optional[str]:
        ...
        if n_angles >= 2:
            ...
            if angle_range >= np.pi / 2:  # 90 degrees or more
                logger.debug(
                    f"Wide angle range ({np.degrees(angle_range):.1f}°) → laminar_flow suggested"
                )
                return "laminar_flow"
            else:
                logger.debug(
                    f"Moderate angle range ({np.degrees(angle_range):.1f}°) → static_anisotropic suggested"
                )
                return "static_anisotropic"
    ...
    def _analyze_data_structure(self, data_dict: Dict[str, Any]) -> Optional[str]:
        ...
        if c2_data is not None:
            ...
            if data_size > 10_000_000:  # 10M points
                logger.debug(
                    f"Large dataset ({data_size:,} points) → laminar_flow might be appropriate"
                )
                return "laminar_flow"
            ...
            # Medium datasets for anisotropic analysis
                ...
                logger.debug(
                    f"Medium dataset ({data_size:,} points) → static_anisotropic suggested"
                )
                return "static_anisotropic"
    ...
    def _validate_and_return_enhanced(
        self,
        mode: str,
        config: Dict[str, Any],
        data_dict: Optional[Dict[str, Any]],
        resolution_results: Dict[str, Optional[str]],
    ) -> str:
        """
            ...
            if data_dict:
                ...
                if not compatibility.is_compatible:
                    ...
                    if compatibility.recommendations:
                        ...
                        logger.info(
                            f"Switching to recommended mode: {best_recommendation}"
                        )
                        return best_recommendation
                elif (
                    compatibility.confidence
                    < self.compatibility_thresholds["medium_confidence"]
                ):
                    logger.warning(
                        f"Low confidence ({compatibility.confidence:.2f}) in mode '{mode}'"
                    )
    ...
    def _analyze_comprehensive_compatibility(
        self, mode: str, config: Dict[str, Any], data_dict: Dict[str, Any]
        ...
        phi_compatibility = self._analyze_phi_angle_compatibility(
            mode, data_dict, requirements
        )
        ...
        data_compatibility = self._analyze_data_size_compatibility(
            mode, data_dict, requirements
        )
        ...
        resource_compatibility = self._analyze_resource_compatibility(
            mode, config, data_dict, requirements
        )
    ...
    def _analyze_phi_angle_compatibility(
        self, mode: str, data_dict: Dict[str, Any], requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if n_angles < min_angles:
            ...
            confidence = 0.0
            reasons.append(
                f"Insufficient phi angles: {n_angles} < {min_angles} required for {mode}"
            )
        ...
        # Check maximum angle constraint
            ...
            confidence = 0.0
            reasons.append(
                f"Too many phi angles: {n_angles} > {max_angles} supported for {mode}"
            )
            suggestions.extend(self._suggest_modes_for_angle_count(n_angles))
    ...
    def _analyze_angle_distribution_quality(
        ...
        angle_range = (
            np.max(phi_angles_rad) - np.min(phi_angles_rad)
            if len(phi_angles_rad) > 1
            else 0
        )
        ...
        if angle_range < np.pi / 6:  # Less than 30 degrees
            quality_score *= 0.7
            observations.append(
                f"Limited angular coverage ({np.degrees(angle_range):.1f}°)"
            )
        elif angle_range > 3 * np.pi / 2:  # More than 270 degrees
            quality_score *= 1.2  # Bonus for good coverage
            observations.append(
                f"Excellent angular coverage ({np.degrees(angle_range):.1f}°)"
            )
    ...
    def _analyze_data_size_compatibility(
        self, mode: str, data_dict: Dict[str, Any], requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        return {"confidence": confidence, "reasons": reasons, "warnings": warnings}
    ...
    def _analyze_resource_compatibility(
        ...
        estimated_memory_gb = (
            data_size * memory_factor * 8 / (1024**3)
        )  # rough estimate
    ...
    def _generate_mode_alternatives(
    ...
    def _provide_performance_recommendations(
        self, mode: str, config: Dict[str, Any], data_dict: Optional[Dict[str, Any]]
    ) -> None:
        """
        ...
        if data_size > 10_000_000:
            ...
            if mode == "laminar_flow":
                logger.info(
                    "  - For laminar flow: reduce MCMC draws to 3000-4000 for faster analysis"
                )
    ...
    def _check_mode_data_compatibility(
        self, mode: str, data_dict: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        if (
            ...
            reason = (
                ...
                else "Mode incompatible with data"
            )
            suggested_mode = (
    ...
    def _log_resolution_summary(
        self, final_method: str, final_mode: str, all_results: Dict[str, Optional[str]]
    ) -> None:
        """
    ...
    def suggest_mode_for_data(
        self, data_dict: Dict[str, Any], config: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if alternatives:
                ...
                primary_analysis = suggestions["detailed_analysis"].get(
                ...
                if (
                    primary_confidence
                    >= self.compatibility_thresholds["high_confidence"]
                ):
    ...
    def _add_performance_notes(
        self, data_dict: Dict[str, Any], suggestions: Dict[str, Any]
    ) -> None:
        """
    ...
    def analyze_mode_transition_feasibility(
        self,
        current_mode: str,
        target_mode: str,
        data_dict: Dict[str, Any],
        ...
        target_compatibility = self._analyze_comprehensive_compatibility(
            target_mode, config, data_dict
        )
        ...
        if analysis["feasible"]:
            self._generate_transition_recommendations(
                current_mode, target_mode, config, analysis
            )
    ...
    def _generate_transition_recommendations(
        ...
        if current_mode == "static_isotropic" and target_mode in [
            "static_anisotropic",
            "laminar_flow",
        ]:
            recommendations.append(
                "Enable angle filtering in optimization configuration"
            )
            recommendations.append(
                "Ensure multiple phi angles are available in your data"
            )

        if target_mode == "laminar_flow":
            recommendations.append(
                "Increase MCMC sampling parameters (draws, tune) for better convergence"
            )
            recommendations.append("Consider using hybrid optimization method")
            recommendations.append(
                "Ensure adequate computational resources (memory, CPU cores)"
            )

        if current_mode == "laminar_flow" and target_mode in [
            "static_isotropic",
            "static_anisotropic",
        ]:
            recommendations.append(
                "Reduce optimization complexity - fewer parameters to fit"
            )
            recommendations.append(
                "Consider reducing MCMC parameters for faster analysis"
            )

        # Configuration adjustments
        opt_config = config.get("optimization_config", {})
        if target_mode == "laminar_flow" and not opt_config.get(
            "mcmc_sampling", {}
        ).get("enabled", True):
    ...
    def _ensure_radians(self, angles: np.ndarray) -> np.ndarray:
        ...
        if np.max(np.abs(angles)) > 2 * np.pi:
            logger.debug(
                f"Converting angles from degrees to radians (max: {np.max(angles):.1f}°)"
            )
            return np.radians(angles)
        else:
            logger.debug(
                f"Angles appear to be in radians (max: {np.max(angles):.3f} rad)"
            )
            return angles
