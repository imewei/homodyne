...
class ParameterBounds:
    ...
    def __post_init__(self):
        ...
        if self.min >= self.max:
            raise ValueError(f"Invalid bounds for {self.name}: min={self.min} >= max={self.max}")
...
class PhysicsParameters:
    ...
    parameter_names: List[str] = field(default_factory=lambda: ["D0", "alpha", "D_offset"])
    ...
    def _validate_parameters(self):
        ...
        if self.mode == AnalysisMode.LAMINAR_FLOW:
            if any(param is None for param in [self.gamma_dot_t0, self.beta, self.gamma_dot_t_offset, self.phi0]):
    ...
    def _update_parameter_names(self):
        ...
        if self.mode == AnalysisMode.STATIC_ISOTROPIC or self.mode == AnalysisMode.STATIC_ANISOTROPIC:
            self.parameter_names = ["D0", "alpha", "D_offset"]
        elif self.mode == AnalysisMode.LAMINAR_FLOW:
            self.parameter_names = ["D0", "alpha", "D_offset", "gamma_dot_t0", "beta", "gamma_dot_t_offset", "phi0"]
    ...
    def to_array(self) -> List[float]:
        ...
        if self.mode == AnalysisMode.STATIC_ISOTROPIC or self.mode == AnalysisMode.STATIC_ANISOTROPIC:
            ...
            return [
                self.D0, self.alpha, self.D_offset,
                self.gamma_dot_t0, self.beta, self.gamma_dot_t_offset, self.phi0
            ]
        else:
    ...
    def from_array(self, values: List[float], mode: AnalysisMode = None):
        ...
        if len(values) >= 7:
            ...
            raise ValueError(f"Laminar flow mode requires 7 parameters, got {len(values)}")
...
class ScalingParameters:
    ...
    def validate_scaling_parameters(self):
        ...
        if len(self.contrasts) != self.n_angles:
            raise ValueError(f"Contrasts length {len(self.contrasts)} != n_angles {self.n_angles}")
        if len(self.offsets) != self.n_angles:
            raise ValueError(f"Offsets length {len(self.offsets)} != n_angles {self.n_angles}")
        
        # Check bounds
        for i, contrast in enumerate(self.contrasts):
            if not 0.01 <= contrast <= 1.0:
                logger.warning(f"Contrast[{i}]={contrast} outside typical range [0.01, 1.0]")
...
def create_parameters_from_config(config_dict: Dict[str, Any], mode: AnalysisMode, n_angles: int = 1) -> CombinedParameters:
    """
...
def validate_parameter_configuration(config_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if values and parameter_names:
        if len(values) != len(parameter_names):
            errors.append(f"Parameter count mismatch: {len(values)} values vs {len(parameter_names)} names")
    ...
    if values and len(values) != expected_count:
        ...
        errors.append(f"Parameter count mismatch for {mode_str} mode: expected {expected_count}, got {len(values)}")
    ...
    for i, (value, name) in enumerate(zip(values, parameter_names)):
        if name in default_bounds:
            ...
            if not bound.min <= value <= bound.max:
                warnings.append(f"{name}={value} outside recommended bounds [{bound.min}, {bound.max}]")
