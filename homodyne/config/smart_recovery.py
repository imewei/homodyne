...
class SmartRecoveryEngine:
    ...
    def __init__(self, config_path: Union[str, Path], interactive_mode: bool = True):
        ...
        self.suggestion_engine = (
            ConfigurationSuggestionEngine() if HAS_EXCEPTIONS else None
        )
    ...
    def _register_default_strategies(self):
        ...
        self.strategies.append(
            RecoveryStrategy(
                ...
                applicability_check=lambda e, c: "yaml"
                in str(type(e).__name__).lower(),
        ...
        self.strategies.append(
            RecoveryStrategy(
                ...
                applicability_check=lambda e, c: any(
                    key in str(e).lower() for key in ["invalid", "unknown", "key"]
                ),
        ...
        self.strategies.append(
            RecoveryStrategy(
                ...
                applicability_check=lambda e, c: "complex" in str(e).lower()
                or len(str(e)) > 500,
        ...
        for strategy in applicable_strategies:
            ...
            try:
                ...
                if result.success:
                    ...
                    logger.warning(
                        f"Recovery strategy failed: {strategy.name} - {result.description}"
                    )

            except Exception as recovery_error:
                logger.error(
                    f"Recovery strategy {strategy.name} raised exception: {recovery_error}"
                )
    ...
    def _ask_user_permission(self, strategy: RecoveryStrategy) -> bool:
        ...
        if strategy.requires_backup:
            print(
                "⚠️  This will modify your configuration file (backup will be created)"
            )

        while True:
            response = (
                input("Proceed with this recovery strategy? [y/N/s(skip)]: ")
                .lower()
                .strip()
            )
    ...
    def _create_backup(self) -> Optional[Path]:
        ...
        backup_name = (
            f"{self.config_path.stem}_backup_{timestamp}{self.config_path.suffix}"
        )
    ...
    def _create_from_template(
        self, error: Exception, context: Dict[str, Any], config_path: Path
    ) -> RecoveryResult:
            ...
            if param_name == "wavevector_q":
                if param_value <= 0:
                    ...
                    corrections_made.append(
                        f"Set wavevector_q to {new_value} (typical XPCS value)"
                    )
                    ...
                    corrections_made.append(
                        f"Reduced wavevector_q to {new_value} (within typical range)"
                    )
                ...
                if param_value <= 0:
                    ...
                    self._set_nested_value(
                        config_data,
                        ["analyzer_parameters", "temporal", "dt"],
                        new_value,
                    )
                    corrections_made.append(f"Set dt to {new_value} seconds")

            elif param_name in ["start_frame", "end_frame"]:
                start = self._get_nested_value(
                    config_data, ["analyzer_parameters", "temporal", "start_frame"], 1
                )
                end = self._get_nested_value(
                    config_data, ["analyzer_parameters", "temporal", "end_frame"], 100
                )
                ...
                if start >= end:
                    ...
                    self._set_nested_value(
                        config_data,
                        ["analyzer_parameters", "temporal", "end_frame"],
                        new_end,
                    )
                    corrections_made.append(
                        f"Set end_frame to {new_end} (start_frame + 1000)"
                    )
                ...
                if abs(param_value) > 2.0:
                    ...
                    corrections_made.append(
                        f"Set alpha to {new_value} (within [-2, 2])"
                    )
    ...
    def _fix_mode_mismatch(
        ...
        try:
            ...
            if param_count == 3:
                # Set to static mode
                self._set_nested_value(
                    config_data, ["analysis_settings", "static_mode"], True
                )
                self._set_nested_value(
                    config_data, ["analysis_settings", "static_submode"], "anisotropic"
                )
                ...
                if self.interactive_mode:
                    ...
                    if choice == "1":
                        self._set_nested_value(
                            config_data,
                            ["analysis_settings", "static_submode"],
                            "isotropic",
                        )
                        self._set_nested_value(
                            config_data,
                            ["optimization_config", "angle_filtering", "enabled"],
                            False,
                        )
                ...
                self._set_nested_value(
                    config_data, ["analysis_settings", "static_mode"], False
                )
                ...
                corrections_made.append(
                    f"Adjusted parameter count from {param_count} to 3"
                )
                corrections_made.append("Set to static anisotropic mode")
    ...
    def _add_missing_sections(
        ...
        try:
            ...
            required_sections = {
                ...
                "logging": {"log_to_console": True, "level": "INFO"},
            }
    ...
    def _fix_invalid_keys(
        ...
        try:
            ...
            suggestions = self.suggestion_engine.generate_autocorrection_suggestions(
                config_data
            )
            corrections_made = []
            ...
            if isinstance(analysis_settings, dict):
                ...
                if mode in self.suggestion_engine.mode_aliases:
                    ...
                    corrections_made.append(
                        f"Changed mode from '{mode}' to '{correct_mode}'"
                    )
        ...
        try:
            ...
            if self.interactive_mode:
                ...
                if config_data:
                    ...
                    print(
                        "Current configuration is complex. Which sections would you like to preserve?"
                    )
                    ...
                    if "1" in preserve_sections and "experimental_data" in config_data:
                        new_config["experimental_data"] = config_data[
                            "experimental_data"
                        ]
                    ...
                    if "3" in preserve_sections and "initial_parameters" in config_data:
                        new_config["initial_parameters"] = config_data[
                            "initial_parameters"
                        ]

                    # Convert back to string
                    if HAS_YAML:
                        simplified_config = yaml.dump(
            ...
            return RecoveryResult(
                ...
                warnings=[
                    "Complex settings were removed - you may need to re-add specific customizations"
                ],
            )
...
logging:
  ...
  level: "INFO"
    ...
    def _update_parameter_in_list(
        self, config_data: Dict[str, Any], param_name: str, new_value: float
    ) -> None:
...
def create_smart_recovery_engine(
    config_path: Union[str, Path], interactive: bool = True
) -> SmartRecoveryEngine:
    """
...
def auto_recover_configuration(
