...
def detect_system_cuda() -> Dict[str, any]:
    ...
    try:
        ...
        cuda_info['cuda_home'] = os.environ.get('CUDA_HOME') or os.environ.get('CUDA_PATH')
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if cuda_info['devices']:
                    cuda_info['memory_total'] = sum(d['memory_mb'] for d in cuda_info['devices'])
                    cuda_info['compute_capability'] = cuda_info['devices'][0]['compute_capability']
        ...
        try:
            ...
            if device_id is not None:
                if device_id < len(devices):
                    ...
                    logger.warning(f"Device ID {device_id} not available, using default")
            ...
            logger.info(f"✓ System CUDA configured successfully with {len(devices)} device(s)")
            logger.info(f"Memory fraction: {memory_fraction:.1%}, "
...
def _configure_cpu_fallback(config_summary: Dict, error: Optional[str] = None) -> Dict:
...
def get_gpu_memory_info() -> Dict[str, any]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            # Calculate totals
            memory_info['total_memory_mb'] = sum(d['total_mb'] for d in memory_info['devices'])
            memory_info['used_memory_mb'] = sum(d['used_mb'] for d in memory_info['devices'])
            memory_info['available_memory_mb'] = sum(d['free_mb'] for d in memory_info['devices'])
...
def optimize_gpu_memory(
    ...
    logger.info(f"GPU memory optimization: batch_size={optimal_batch_size}, "
...
def benchmark_gpu_performance(
    ...
    for test_size in test_sizes:
        ...
        logger.info(f"Size {test_size}: {mean_time:.4f}s ± {std_time:.4f}s "
    ...
    if results['test_results']:
        ...
        results['memory_bandwidth'] = bytes_processed / largest_result['mean_time'] / 1e9  # GB/s

    logger.info(f"Benchmark completed. Peak throughput: {results['peak_throughput']:.0f} ops/sec")
...
def validate_cuda_installation() -> Dict[str, any]:
    ...
    try:
        ...
        if JAX_AVAILABLE:
            ...
            validation['recommendations'].append("Install JAX with: pip install jax[local]")
