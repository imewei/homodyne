...
try:
    ...
    VI_AVAILABLE = True
    VI_FALLBACK_MODE = "numpy_gradients" if not VI_JAX_AVAILABLE and VI_HAS_NUMPY_GRADIENTS else ("simple" if not VI_JAX_AVAILABLE else "jax")
...
try:
    from homodyne.optimization.mcmc import (
        MCMCJAXSampler,
        MCMCResult, 
        fit_mcmc_jax,  # Primary API function
        JAX_AVAILABLE as MCMC_JAX_AVAILABLE,
        HAS_NUMPY_GRADIENTS as MCMC_HAS_NUMPY_GRADIENTS
    )
    MCMC_AVAILABLE = True
    MCMC_FALLBACK_MODE = "numpy_mh" if not MCMC_JAX_AVAILABLE and MCMC_HAS_NUMPY_GRADIENTS else ("unavailable" if not MCMC_JAX_AVAILABLE else "jax")
except ImportError as e:
...
def fit_homodyne_vi(data, sigma, t1, t2, phi, q, L, analysis_mode="laminar_flow", **kwargs):
    """
    ...
    if VI_FALLBACK_MODE == "numpy_gradients":
        ...
        print("WARNING: Using simple fallback mode - limited accuracy. Consider installing JAX.")
    
    return fit_vi_jax(data, sigma, t1, t2, phi, q, L, analysis_mode, **kwargs)

def fit_homodyne_mcmc(data, sigma, t1, t2, phi, q, L, analysis_mode="laminar_flow", **kwargs):
    """
    ...
    if MCMC_FALLBACK_MODE == "numpy_mh":
        print("INFO: Using NumPy+Metropolis-Hastings fallback (50-200x slower than JAX+NUTS)")
