...
def compute_numpy_elbo(
    ...
    try:
        ...
        log_likelihood = -0.5 * np.sum(residuals**2) - 0.5 * np.sum(
    ...
    for i, (prior_mu, prior_std) in enumerate(param_priors):
        ...
        kl_i = (
            np.log(prior_std / param_std[i])
            + (param_std[i] ** 2 + (param_mu[i] - prior_mu) ** 2) / (2 * prior_std**2)
            - 0.5
        )
        kl_divergence += kl_i
    ...
    kl_contrast = (
        np.log(prior_contrast_std / contrast_std)
        + (contrast_std**2 + (contrast_mu - prior_contrast_mu) ** 2)
        / (2 * prior_contrast_std**2)
        - 0.5
    )
    ...
    kl_offset = (
        np.log(prior_offset_std / offset_std)
        + (offset_std**2 + (offset_mu - prior_offset_mu) ** 2)
        / (2 * prior_offset_std**2)
        - 0.5
    )
...
def unflatten_variational_params(
    flat_params: np.ndarray, n_params: int
) -> Dict[str, np.ndarray]:
