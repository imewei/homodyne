...
class ResultExporter:
    ...
    def _extract_result_data(self, result: ResultType) -> Dict[str, Any]:
        ...
        if hasattr(result, "mean_params"):
            data["parameters"] = {
                ...
                "count": (
                    len(result.mean_params)
                    if hasattr(result.mean_params, "__len__")
                    else 0
                ),
            }
        ...
        if hasattr(result, "std_params"):
            data["parameters"]["std"] = (
                result.std_params.tolist()
                if hasattr(result.std_params, "tolist")
                else result.std_params
            )
    ...
    def _extract_mcmc_data(self, result: MCMCResult) -> Dict[str, Any]:
        ...
        if hasattr(result, "r_hat"):
            mcmc_data["mcmc_specific"]["r_hat"] = (
                result.r_hat.tolist()
                if hasattr(result.r_hat, "tolist")
                else result.r_hat
            )

        if hasattr(result, "ess"):
            mcmc_data["mcmc_specific"]["ess"] = (
                result.ess.tolist() if hasattr(result.ess, "tolist") else result.ess
            )
...
class YAMLExporter(ResultExporter):
    ...
    def export(self, result: ResultType, filename: Optional[str] = None) -> Path:
        ...
        filename = (
            filename
            or f"analysis_result_{self.export_timestamp.strftime('%Y%m%d_%H%M%S')}.yaml"
        )
...
class JSONExporter(ResultExporter):
    ...
    def export(
        self, result: ResultType, filename: Optional[str] = None, pretty: bool = True
    ) -> Path:
        """
        ...
        filename = (
            filename
            or f"analysis_result_{self.export_timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        )
        output_path = self.output_dir / filename
...
class NPZExporter(ResultExporter):
    ...
    def export(self, result: ResultType, filename: Optional[str] = None) -> Path:
        ...
        filename = (
            filename
            or f"analysis_result_{self.export_timestamp.strftime('%Y%m%d_%H%M%S')}.npz"
        )
        output_path = self.output_dir / filename
        ...
        if "vi_specific" in data and "elbo_history" in data["vi_specific"]:
            arrays_to_save["elbo_history"] = np.array(
                data["vi_specific"]["elbo_history"]
            )
...
class HDF5Exporter(ResultExporter):
    ...
    def export(self, result: ResultType, filename: Optional[str] = None) -> Path:
        ...
        filename = (
            filename
            or f"analysis_result_{self.export_timestamp.strftime('%Y%m%d_%H%M%S')}.h5"
        )
        output_path = self.output_dir / filename
...
class CSVExporter(ResultExporter):
    ...
    def export(self, result: ResultType, filename: Optional[str] = None) -> Path:
        ...
        filename = (
            filename
            or f"analysis_result_{self.export_timestamp.strftime('%Y%m%d_%H%M%S')}.csv"
        )
        output_path = self.output_dir / filename
...
class MultiFormatExporter:
    ...
    def export_all_formats(
        self, result: ResultType, formats: Optional[List[str]] = None
    ) -> Dict[str, Path]:
        """
    ...
    def export_selective(
        self, result: ResultType, format_preferences: Dict[str, bool]
    ) -> Dict[str, Path]:
        """
        ...
        """
        enabled_formats = [
            fmt for fmt, enabled in format_preferences.items() if enabled
        ]
