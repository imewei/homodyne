    ...
    def _generate_recommendations(self, issues: List[ValidationIssue]) -> List[str]:
        ...
        if category_counts.get("convergence", 0) > 2:
            recommendations.append(
                "Multiple convergence issues detected - consider adjusting optimization settings"
            )

        if category_counts.get("parameters", 0) > 2:
            recommendations.append(
                "Several parameter issues found - check physics constraints and initial conditions"
            )

        if category_counts.get("quality", 0) > 0:
            recommendations.append(
                "Quality metrics suggest checking model appropriateness and data quality"
            )
...
class ResultFormatter:
    ...
    def format_result(
        self, result: ResultType, style: str = "standard"
    ) -> Dict[str, Any]:
        """
        ...
        for i, (name, value) in enumerate(zip(param_names, mean_params)):
            ...
            if hasattr(result, "std_params") and result.std_params is not None:
                ...
                if i < len(std_params):
                    ...
                    param_info["formatted"] = (
                        f"{param_info['value']} ± {param_info['uncertainty']}"
                    )
    ...
    def _get_unit_conversions(self) -> Dict[str, Dict[str, float]]:
        ...
        """
        return {
            "diffusion": {"m²/s_to_μm²/s": 1e12, "cm²/s_to_μm²/s": 1e8},
            "time": {"s_to_ms": 1000, "s_to_μs": 1e6},
        }
