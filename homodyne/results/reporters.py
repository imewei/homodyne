...
<div class="footer">
    ...
    def format_table(
        self, data: Dict[str, Any], caption: str = "", table_class: str = "data-table"
    ) -> str:
    ...
    def format_figure(
        self, figure_path: str, caption: str = "", figure_class: str = "analysis-figure"
    ) -> str:
...
class ReportGenerator:
    ...
    def __init__(self, output_dir: Union[str, Path]):
        ...
        # Available templates
        self.templates = {"markdown": MarkdownTemplate, "html": HTMLTemplate}
    ...
    def generate_report(
        ...
        if config.format == "html":
            return self._generate_html_report(
                result, config, title, summary, plot_paths
            )
    ...
    def _generate_html_report(
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                f'<p class="subtitle">Generated on {datetime.now().strftime("%B %d, %Y at %H:%M:%S")}</p>\n'
            )

            # Summary sections
            for section in summary.sections:
                section_content = self._format_section_content(
                    section, template, plot_paths
                )
    ...
    def _generate_pdf_report(
        ...
        md_config = ReportConfig(
            format="markdown", template_style=config.template_style
        )
        md_file = self._generate_markdown_report(
            result, md_config, title, summary, plot_paths
        )
        ...
        try:
            ...
            logger.warning(
                "Pandoc not available for PDF conversion. Install pandoc for PDF support."
            )
    ...
    def _generate_latex_report(
        ...
        output_file = (
            self.output_dir
            / f"analysis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.tex"
        )
    ...
    def _format_section_content(
        self, section: Any, template: ReportTemplate, plot_paths: Dict[str, str] = None
    ) -> str:
    ...
    def _create_plots_section(
        self, template: ReportTemplate, plot_paths: Dict[str, str]
    ) -> str:
...
"""
    ...
    def _format_latex_content(self, section: Any) -> str:
        ...
        if isinstance(content, dict):
            ...
            content = (
                content.replace("_", "\\_").replace("%", "\\%").replace("&", "\\&")
            )
            return content
        else:
            return str(content)
    ...
    def generate_batch_reports(
        ...
        config: ReportConfig,
        comparison_title: str = "Batch Analysis Report",
    ) -> List[Path]:
        """
