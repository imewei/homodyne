...
class AnalysisSummarizer:
    ...
    def create_summary(
        ...
        quality_section, quality_recommendations = self._create_quality_section(
            result, level
        )
        ...
        if include_validation:
            validation_section, validation_recommendations = (
                self._create_validation_section(result, level)
            )
        ...
        if level != SummaryLevel.EXECUTIVE or all_recommendations:
            sections.append(
                self._create_recommendations_section(all_recommendations, level)
            )
        ...
        logger.info(
            f"✓ Summary created with {len(sections)} sections and {len(key_findings)} key findings"
        )
        return summary

    def _create_overview_section(
        self, result: ResultType, level: SummaryLevel
    ) -> SummarySection:
        """
        ...
        if level == SummaryLevel.EXECUTIVE:
            ...
            content = {
                ...
                "parameter_count": (
                    len(result.mean_params) if hasattr(result, "mean_params") else 0
                ),
        ...
        return SummarySection(title="Analysis Overview", content=content, importance=5)
    ...
    def _create_parameter_section(
        ...
        if level == SummaryLevel.EXECUTIVE:
            ...
            param_summary = self._create_executive_parameter_summary(
                mean_params, param_names
            )
            ...
            key_findings.extend(
                self._identify_notable_parameters(mean_params, param_names)
            )
            ...
            content = {
                ...
                "physical_interpretation": self._create_physical_interpretation(
                    mean_params, param_names
                ),
            }
        ...
        return (
            SummarySection(title="Parameter Analysis", content=content, importance=4),
            key_findings,
        )
    ...
    def _create_quality_section(
        ...
        if hasattr(result, "chi_squared"):
            ...
            if chi_sq > self.quality_thresholds["chi_squared_high"]:
                recommendations.append(
                    "High chi-squared suggests model-data mismatch - consider alternative models"
                )
            elif chi_sq < self.quality_thresholds["chi_squared_low"]:
                recommendations.append(
                    "Very low chi-squared may indicate overfitting - validate with independent data"
                )

        if hasattr(result, "final_elbo"):
            quality_metrics["final_elbo"] = self.formatter._format_number(
                result.final_elbo
            )
        ...
        if isinstance(result, MCMCResult):
            if hasattr(result, "r_hat"):
                r_hat_max = (
                    np.max(result.r_hat)
                    if hasattr(result.r_hat, "__len__")
                    else result.r_hat
                )
                ...
                if r_hat_max > 1.1:
                    recommendations.append(
                        "High R-hat indicates poor chain convergence - increase warmup or samples"
                    )
    ...
    def _create_method_sections(
        self, result: ResultType, level: SummaryLevel
    ) -> Tuple[List[SummarySection], List[str]]:
        """
    ...
    def _create_vi_section(
        self, result: VIResult, level: SummaryLevel
    ) -> Tuple[SummarySection, List[str]]:
        """
        ...
        if level == SummaryLevel.EXECUTIVE:
            ...
            if result.converged:
                ...
                key_findings.append(
                    "VI optimization did not fully converge - consider increasing iterations"
                )
        ...
        return (
            SummarySection(
                title="Variational Inference Analysis", content=content, importance=3
            ),
            key_findings,
        )

    def _create_mcmc_section(
        self, result: MCMCResult, level: SummaryLevel
    ) -> Tuple[SummarySection, List[str]]:
        """
        ...
        if level == SummaryLevel.EXECUTIVE:
            ...
            content = {
                "sampling_quality": (
                    "Good"
                    if max_r_hat and max_r_hat < 1.05
                    else "Acceptable" if max_r_hat and max_r_hat < 1.1 else "Poor"
                ),
            ...
            if hasattr(result, "r_hat") and result.r_hat is not None:
                ...
                if max_r_hat < 1.05:
                    key_findings.append(
                        "Excellent MCMC chain convergence (R-hat < 1.05)"
                    )
                elif max_r_hat < 1.1:
                    key_findings.append("Good MCMC chain convergence (R-hat < 1.1)")
                else:
                    key_findings.append(
                        "Poor MCMC chain convergence (R-hat > 1.1) - longer sampling needed"
                    )
        ...
        return (
            SummarySection(
                title="MCMC Sampling Analysis", content=content, importance=3
            ),
            key_findings,
        )

    def _create_hybrid_sections(
        self, result: HybridResult, level: SummaryLevel
    ) -> Tuple[List[SummarySection], List[str]]:
        """
        ...
        if level == SummaryLevel.EXECUTIVE:
            ...
            content = {
                ...
                "vi_phase_quality": (
                    "Good" if hasattr(result, "vi_result") else "Not available"
                ),
                "mcmc_phase_quality": (
                    "Good" if hasattr(result, "mcmc_result") else "Not available"
                ),
            }

            key_findings.append(
                f"Hybrid analysis recommends {recommendation.upper()} method for final results"
            )
        ...
        sections.append(
            SummarySection(
                title="Hybrid Method Analysis", content=content, importance=4
            )
        )
        ...
        if level == SummaryLevel.DETAILED:
            if hasattr(result, "vi_result") and result.vi_result:
                vi_section, vi_findings = self._create_vi_section(
                    result.vi_result, SummaryLevel.TECHNICAL
                )
            ...
            if hasattr(result, "mcmc_result") and result.mcmc_result:
                mcmc_section, mcmc_findings = self._create_mcmc_section(
                    result.mcmc_result, SummaryLevel.TECHNICAL
                )
    ...
    def _create_validation_section(
        self, result: ResultType, level: SummaryLevel
    ) -> Tuple[Optional[SummarySection], List[str]]:
        """
        ...
        if level == SummaryLevel.EXECUTIVE:
            ...
            critical_issues = [
                i for i in validation_report.issues if i.severity.value == "critical"
            ]
            ...
            content = {
                "validation_status": (
                    "Passed" if validation_report.is_valid else "Issues found"
                ),
    ...
    def _create_recommendations_section(
        self, recommendations: List[str], level: SummaryLevel
    ) -> SummarySection:
        """
        ...
        return SummarySection(title="Recommendations", content=content, importance=3)

    def _consolidate_recommendations(
        self, recommendations: List[str], result: ResultType
    ) -> List[str]:
        """
    ...
    def _generate_general_recommendations(self, result: ResultType) -> List[str]:
        ...
        if hasattr(result, "mean_params"):
            ...
            if param_count == 7:
                recommendations.append(
                    "Complex 7-parameter model - validate results with simpler models"
                )

        # Method-specific general recommendations
        if isinstance(result, VIResult):
            recommendations.append(
                "Consider MCMC validation for critical analyses requiring full uncertainty quantification"
            )
        elif isinstance(result, MCMCResult):
            recommendations.append(
                "MCMC provides full posterior - consider credible intervals for uncertainty assessment"
            )
    ...
    def _get_interpretation_rules(self) -> Dict[str, Any]:
        ...
        return {
            "D₀": {
                "physical_meaning": "Diffusion coefficient amplitude",
                "typical_range": (1e-12, 1e-8),
            },
            "α": {
                "physical_meaning": "Anomalous diffusion exponent",
                "typical_range": (0.5, 1.5),
            },
            "γ̇₀": {
                "physical_meaning": "Shear rate amplitude",
    ...
    def _create_executive_parameter_summary(
        self, params: np.ndarray, names: List[str]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "key_physics": (
                "Anomalous diffusion"
                if len(params) == 3
                else "Laminar flow with diffusion"
            ),
        }

    def _identify_notable_parameters(
        self, params: np.ndarray, names: List[str]
    ) -> List[str]:
        ...
        for name, value in zip(names, params):
            if name == "α" and value < 0.8:
                ...
                findings.append(
                    f"Superdiffusive behavior detected (α = {value:.3f} > 1)"
                )
        return findings
    ...
    def _create_physical_interpretation(
        self, params: np.ndarray, names: List[str]
    ) -> Dict[str, str]:
            ...
            if name == "α":
                if value < 1:
                    interpretations[name] = (
                        "Subdiffusive transport (constrained motion)"
                    )
                elif value > 1:
                    interpretations[name] = (
                        "Superdiffusive transport (enhanced spreading)"
                    )
                else:
