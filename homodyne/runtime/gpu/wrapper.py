...
def setup_gpu_environment(quiet: bool = False) -> bool:
    ...
    try:
        ...
        if platform.system() != "Linux":
            if not quiet:
                print(f"GPU acceleration requires Linux (detected: {platform.system()})")
            return False
    ...
    if success:
        ...
        if not quiet:
            ...
            try:
                ...
                print("�  JAX not available - install with: pip install jax[cuda12-local]")
    else:
        if not quiet:
...
def get_gpu_status() -> Dict[str, Any]:
    ...
    try:
        ...
        status["gpu_available"] = any("gpu" in str(d).lower() or "cuda" in str(d).lower() for d in devices)
    except ImportError:
        pass
