...
try:
    ...
    from homodyne.data.preprocessing import PreprocessingConfig, PreprocessingStage
    from homodyne.data.quality_controller import QualityControlConfig, QualityThreshold
    from homodyne.data.performance_engine import PerformanceConfig, OptimizationLevel
...
class TestConfigurationSchema:
    ...
    def test_complete_enhanced_config_schema(self):
        ...
        assert config_manager.get('data_loading', 'enhanced_features', 'enable_filtering') == True
        assert config_manager.get('data_loading', 'filtering', 'quality_threshold') == 0.85
        assert config_manager.get('data_loading', 'preprocessing', 'diagonal_correction', 'method') == 'statistical'
        assert config_manager.get('data_loading', 'quality_control', 'enable_progressive') == True
        assert config_manager.get('data_loading', 'performance', 'optimization_level') == 'adaptive'
    ...
    def test_quality_control_config_validation(self):
        ...
        for i, config_dict in enumerate(valid_qc_configs):
            ...
            for threshold_name, threshold_value in qc_config.thresholds.items():
                if isinstance(threshold_value, (int, float)):
                    assert threshold_value >= 0.0, f"Threshold {threshold_name} should be non-negative"
                    if 'completeness' in threshold_name or 'stability' in threshold_name:
                        assert threshold_value <= 1.0, f"Threshold {threshold_name} should be <= 1.0"
        ...
        invalid_qc_configs = [
            {
                'thresholds': {
                    'signal_to_noise': -1.0  # Negative SNR threshold
                }
            },
            {
                'thresholds': {
                    'data_completeness': 1.5  # > 1.0
                }
            },
            {
                'repair_strategies': {
...
class TestConfigurationTemplates:
        ...
        templates = {
            ...
            'performance_optimized.yaml': {
                'data_loading': {
                    ...
                    'performance': {
                        'optimization_level': 'aggressive',
                        'caching': {'enable_memory_cache': True, 'enable_disk_cache': True},
                        'parallel_processing': {'enable': True},
    ...
    def test_template_validity(self):
        ...
        for template_name in self.template_files:
            ...
            assert 'data_loading' in config_data, f"Template {template_name} missing data_loading section"
            
            # Test ConfigManager parsing
            config_manager = ConfigManager(str(template_path))
            
            # Verify basic structure
            enhanced_features = config_manager.get('data_loading', 'enhanced_features', default={})
            assert isinstance(enhanced_features, dict), f"Template {template_name} has invalid enhanced_features"
    ...
    def test_template_completeness(self):
        ...
        required_sections = {
            ...
            'standard_enhanced.yaml': ['enhanced_features', 'filtering', 'preprocessing', 'quality_control'],
            'performance_optimized.yaml': ['enhanced_features', 'filtering', 'preprocessing', 'quality_control', 'performance'],
            'comprehensive_enhanced.yaml': ['enhanced_features', 'filtering', 'preprocessing', 'quality_control', 'performance']
        }
        ...
        for template_name, expected_sections in required_sections.items():
            ...
            for section in expected_sections:
                ...
                assert section_data, f"Template {template_name} missing or empty section: {section}"
    ...
    def test_template_consistency(self):
        ...
        for template_name, config_manager in template_configs.items():
            ...
            quality_threshold = config_manager.get('data_loading', 'filtering', 'quality_threshold', default=None)
            if quality_threshold is not None:
                assert 0.0 <= quality_threshold <= 1.0, f"Template {template_name} has invalid quality threshold"
            
            # Check preprocessing stages are valid
            preprocessing_stages = config_manager.get('data_loading', 'preprocessing', 'pipeline_stages', default=[])
            valid_stages = ['diagonal_correction', 'normalization', 'noise_reduction', 'outlier_treatment']
            for stage in preprocessing_stages:
                assert stage in valid_stages, f"Template {template_name} has invalid preprocessing stage: {stage}"
            
            # Check performance optimization levels
            opt_level = config_manager.get('data_loading', 'performance', 'optimization_level', default=None)
            if opt_level is not None:
                valid_levels = ['conservative', 'balanced', 'aggressive', 'adaptive']
                assert opt_level in valid_levels, f"Template {template_name} has invalid optimization level"
    ...
    def test_template_documentation_accuracy(self):
        ...
        enhanced_features = minimal_config.get('data_loading', 'enhanced_features', default={})
        
        # Should only have filtering enabled
        assert enhanced_features.get('enable_filtering', False) == True
        assert enhanced_features.get('enable_preprocessing', True) == False  # Default should be False
        ...
        perf_config = ConfigManager(str(self.template_dir / 'performance_optimized.yaml'))
        enhanced_features = perf_config.get('data_loading', 'enhanced_features', default={})
        ...
        assert performance_section, "Performance template missing performance configuration"
...
class TestConfigurationMigration:
    ...
    def _create_old_format_configs(cls):
        ...
        cls.old_configs = {
            'json_v1': old_json_path,
            'yaml_early_v2': old_yaml_path
        }
    ...
    def test_json_to_yaml_migration(self):
        ...
        migrated_config = {
            'data_loading': {
                'enhanced_features': {
                    'enable_filtering': old_config.get('xpcs_loader', {}).get('enable_filtering', False),
                    ...
                    'enable_performance_optimization': old_config.get('performance', {}).get('enable_caching', False)
                },
                'filtering': {
                    'quality_threshold': old_config.get('xpcs_loader', {}).get('quality_threshold', 0.8),
                    'q_range': {
                        'min': old_config.get('xpcs_loader', {}).get('q_range', [1e-4, 1e-2])[0],
                        'max': old_config.get('xpcs_loader', {}).get('q_range', [1e-4, 1e-2])[1]
                    }
                },
                'preprocessing': {
                    'pipeline_stages': [],  # Build from old boolean flags
                    'diagonal_correction': {'method': 'statistical'},  # Default modern method
                    'normalization': {'method': old_config.get('preprocessing', {}).get('normalization', 'baseline')}
                },
                'performance': {
                    'optimization_level': 'balanced',  # Conservative migration
                    'parallel_processing': {
                        'enable': True,
                        'max_threads': old_config.get('performance', {}).get('max_threads', 4)
                    },
                    'caching': {
                        'enable_memory_cache': old_config.get('performance', {}).get('enable_caching', False)
                    }
                }
            }
        }
        ...
        if old_config.get('preprocessing', {}).get('diagonal_correction', False):
            migrated_config['data_loading']['preprocessing']['pipeline_stages'].append('diagonal_correction')
        if old_config.get('preprocessing', {}).get('normalization'):
            migrated_config['data_loading']['preprocessing']['pipeline_stages'].append('normalization')
        if old_config.get('preprocessing', {}).get('noise_reduction', False):
            migrated_config['data_loading']['preprocessing']['pipeline_stages'].append('noise_reduction')
        ...
        assert config_manager.get('data_loading', 'enhanced_features', 'enable_filtering') == True
        assert config_manager.get('data_loading', 'filtering', 'quality_threshold') == 0.8
    ...
    def test_backward_compatibility(self):
        ...
        mixed_config = {
            'data_loading': {
                ...
                'preprocessing': {
                    'stages': ['diagonal', 'normalize'],  # Old stage names
                    'pipeline_stages': ['diagonal_correction']  # New format - should take precedence
                }
            }
        }
    ...
    def test_configuration_version_detection(self):
        ...
        configs_to_test = [
            ...
            {
                'data_loading': {
                    'filtering': {'threshold': 0.8}  # Old parameter name
                }
            }
        ]
        ...
        for i, (config_data, expected_version) in enumerate(zip(configs_to_test, expected_versions)):
            ...
            if 'data_loading' in config_data and 'enhanced_features' in config_data['data_loading']:
                detected_version = config_data['data_loading']['enhanced_features'].get('version', '2.0.0')
            elif 'xpcs_loader' in config_data:
                detected_version = '1.0.0'  # Old format
            else:
                detected_version = 'mixed'
            
            assert detected_version == expected_version, f"Version detection failed for config {i}"
...
class TestConfigurationInheritanceAndOverrides:
    ...
    def test_configuration_defaults(self):
        ...
        preprocessing_config_dict = config_manager.get('data_loading', 'preprocessing', default={})
    ...
    def test_configuration_overrides(self):
        ...
        override_config = {
            'data_loading': {
                'filtering': {
                    'quality_threshold': 0.9  # Override only this value
                },
                'preprocessing': {
                    'pipeline_stages': ['diagonal_correction', 'normalization']  # Add stage
                }
            }
        }
        ...
        assert config_manager.get('data_loading', 'filtering', 'quality_threshold') == 0.9  # Overridden
        assert config_manager.get('data_loading', 'filtering', 'q_range', 'min') == 1e-4  # From base
        
        # Preprocessing should have both stages
        stages = config_manager.get('data_loading', 'preprocessing', 'pipeline_stages', default=[])
    ...
    def test_environment_variable_overrides(self):
        ...
        with patch.dict(os.environ, env_overrides):
            ...
            env_quality_threshold = os.environ.get('HOMODYNE_FILTERING_QUALITY_THRESHOLD')
