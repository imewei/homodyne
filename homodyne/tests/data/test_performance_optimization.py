...
try:
    from homodyne.data.performance_engine import (
        PerformanceEngine, PerformanceMetrics, MemoryMapManager,
        AdaptiveChunker, MultiLevelCache, ChunkInfo,
        PerformanceEngineError, MemoryPressureError, CacheError
    )
...
try:
    from homodyne.data.memory_manager import (
        AdvancedMemoryManager, MemoryPressureMonitor, MemoryStats,
        MemoryPool, MemoryManagerError, MemoryPressureError as MemMgrMemoryPressureError,
        AllocationError
    )
...
# Test utilities
def create_test_hdf5_file(file_path: str, num_matrices: int = 10, matrix_size: int = 64) -> None:
...
class TestAdaptiveChunker(unittest.TestCase):
    ...
    def test_performance_feedback_update(self):
        ...
        self.chunker.update_performance_feedback(chunk_info, actual_processing_time=1.5, success=True)
...
class TestPerformanceEngine(unittest.TestCase):
    ...
    def test_cache_key_generation(self):
        ...
        with patch('os.stat') as mock_stat:
            mock_stat.return_value = type('MockStat', (), {'st_mtime': 12345, 'st_size': 67890})()
...
@unittest.skipUnless(HAS_ADVANCED_OPTIMIZER and HAS_PERFORMANCE_ENGINE, "Advanced optimizer not available")
...
class TestIntegrationWithXPCSLoader(unittest.TestCase):
    ...
    def test_performance_components_initialization(self, mock_optimizer, mock_memory, mock_engine):
...
"""
    ...
    def test_performance_config_validation(self):
        ...
        self.assertGreater(memory_config['critical_threshold'], memory_config['warning_threshold'])
        
        # Validate cache settings
        cache_config = valid_config['performance']['caching']
        self.assertGreater(cache_config['memory_cache_mb'], 0)
        self.assertIn(cache_config['compression_level'], range(1, 23))  # Valid range for zstd
