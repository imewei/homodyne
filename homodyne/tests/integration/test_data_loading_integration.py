...
try:
    ...
    from homodyne.data.filtering_utils import (FilterConfig, FilteringEngine,
                                               FilterType, FrameFilter,
                                               PhiFilter, QRangeFilter,
                                               QualityFilter)
    ...
    from homodyne.data.performance_engine import (OptimizationLevel,
                                                  PerformanceConfig,
                                                  PerformanceEngine)
    from homodyne.data.preprocessing import (DiagonalCorrectionMethod,
                                             NoiseReductionMethod,
                                             NormalizationMethod,
                                             PreprocessingConfig,
                                             PreprocessingPipeline,
                                             PreprocessingStage)
    from homodyne.data.quality_controller import (QualityAction,
...
class TestDataLoadingIntegration:
    ...
    def test_basic_integration_pipeline(self):
        ...
        assert (
            filtered_data["g2"].shape[-1] == raw_data["g2"].shape[-1]
        )  # Time delays preserved
    ...
    def test_performance_optimization_integration(self):
        ...
        print(
            f"✓ Performance optimization integration: {processing_time:.2f}s, {memory_usage / 1024**2:.1f}MB"
        )
    ...
    def test_cross_format_consistency(self):
        ...
        results = {}
        for format_name, dataset_path in [
            ("aps_old", aps_old_path),
            ("aps_u", aps_u_path),
        ]:
            loader = XPCSLoader(dataset_path)
            data = loader.load_data()
    ...
    def test_error_handling_and_recovery(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            assert any(
                "quality" in msg.lower() or "corrupt" in msg.lower()
    ...
    def test_data_integrity_round_trip(self):
        ...
        for q_idx in range(processed_g2.shape[0]):
            for phi_idx in range(processed_g2.shape[1]):
                ...
                assert (
                    g2_trace[0] >= g2_trace[-1]
                ), f"Correlation should decay for q={q_idx}, phi={phi_idx}"
        ...
        assert (
            std_g2 < 2.0
        ), f"g2 standard deviation ({std_g2:.3f}) should not be excessive"
...
class TestComponentInteractions:
    ...
    def test_quality_control_preprocessing_feedback(self):
        ...
        dataset_path = self.test_datasets.get(
            "edge_case_noisy", self.test_datasets["integration_small"]
        )
        ...
        preprocessing_config = PreprocessingConfig(
            pipeline_stages=[PreprocessingStage.NOISE_REDUCTION], quality_feedback=True
        )
        preprocessing_pipeline = PreprocessingPipeline(preprocessing_config)
    ...
    def test_performance_memory_coordination(self):
        ...
        memory_config = MemoryConfig(
            strategy=MemoryStrategy.ADAPTIVE, coordinate_with_performance_engine=True
        )
        ...
        print(
            f"✓ Performance-memory coordination validated: {memory_usage / 1024**2:.1f}MB"
        )
...
def test_suite_completeness():
    ...
    test_methods = [
        method
        for method in dir(TestDataLoadingIntegration)
        if method.startswith("test_")
    ]

    assert (
        len(test_methods) >= 10
    ), f"Should have at least 10 integration tests, found {len(test_methods)}"
    ...
    for pattern in key_test_patterns:
        matching_tests = [
            method for method in test_methods if pattern in method.lower()
        ]
