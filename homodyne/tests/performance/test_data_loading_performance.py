...
from homodyne.tests.data.synthetic_data_generator import (
    DataQuality, DatasetFormat, DatasetSize, SyntheticDataGenerator,
    SyntheticDatasetConfig, generate_test_dataset_suite)

# Core homodyne imports
try:
    from homodyne.data.filtering_utils import FilterConfig, FilteringEngine
    from homodyne.data.memory_manager import (MemoryConfig, MemoryManager,
                                              MemoryStrategy)
...
class TestPerformanceBenchmarks:
    ...
    def test_data_loading_performance(self):
        ...
        for size_name, dataset_key in test_cases:
            ...
            if "time" in baseline:
                ...
                assert (
                    time_ratio < 2.0
                ), f"Loading {size_name} dataset too slow: {metrics.execution_time:.2f}s vs {baseline['time']}s baseline"

            if "memory" in baseline:
                memory_ratio = metrics.peak_memory_mb / baseline["memory"]
                assert (
                    memory_ratio < 2.0
                ), f"Loading {size_name} dataset uses too much memory: {metrics.peak_memory_mb:.1f}MB vs {baseline['memory']}MB baseline"

            results[size_name] = metrics
            print(
                f"✓ {size_name.capitalize()} dataset: {metrics.execution_time:.2f}s, {metrics.peak_memory_mb:.1f}MB, {metrics.throughput_mbps:.1f}MB/s"
            )
        ...
        if "small" in results and "large" in results:
            ...
            assert (
                scaling_factor < 1.0
            ), f"Loading time scales poorly: {time_ratio:.2f}x time for {size_ratio:.2f}x size"

            print(
                f"✓ Loading scalability: {time_ratio:.2f}x time for {size_ratio:.2f}x size"
            )
    ...
    def test_filtering_performance(self):
        ...
        filter_configs = [
            ...
            (
                "quality_filtering",
                FilterConfig(quality_threshold=0.95, enable_advanced_quality=True),
            ),
        ]
        ...
        for config_name, filter_config in filter_configs:
            ...
            ), f"Filtering too slow: {metrics.execution_time:.2f}s"

            # Should not use excessive memory
            memory_overhead = metrics.peak_memory_mb / dataset_size_mb
            assert (
                memory_overhead < 5.0
            ), f"Filtering memory overhead too high: {memory_overhead:.1f}x"

            print(
                f"✓ {config_name}: {metrics.execution_time:.2f}s, {metrics.peak_memory_mb:.1f}MB"
            )
    ...
    def test_preprocessing_performance(self):
        ...
        for config_name, preprocessing_config in preprocessing_configs:
            ...
            ), f"Preprocessing too slow: {metrics.execution_time:.2f}s for {stage_count} stages"

            print(
                f"✓ {config_name} preprocessing: {metrics.execution_time:.2f}s, {metrics.peak_memory_mb:.1f}MB"
            )
    ...
    def test_memory_optimization_effectiveness(self):
        ...
        for strategy_name, strategy in memory_strategies:
            ...
            assert (
                metrics.peak_memory_mb < memory_limit_mb * 1.2
            ), f"{strategy_name} strategy exceeded memory limit: {metrics.peak_memory_mb:.1f}MB"

            print(
                f"✓ {strategy_name} strategy: {metrics.execution_time:.2f}s, {metrics.peak_memory_mb:.1f}MB"
            )
        ...
        if "conservative" in results and "aggressive" in results:
            ...
            assert (
                conservative.peak_memory_mb <= aggressive.peak_memory_mb
            ), "Conservative strategy should use less memory"
            ...
            assert (
                time_ratio < 1.5
            ), f"Aggressive strategy should not be much slower: {time_ratio:.2f}x"

            print(
                f"✓ Memory strategy comparison: Conservative={conservative.peak_memory_mb:.1f}MB, Aggressive={aggressive.peak_memory_mb:.1f}MB"
            )
    ...
    def test_parallel_processing_scalability(self):
        ...
        for thread_count in thread_counts:
            ...
            with performance_monitor(operation_name, dataset_size_mb):
                filter_config = FilterConfig(
                    enable_parallel=thread_count > 1, max_threads=thread_count
                )
                filtering_engine = FilteringEngine(filter_config)
            ...
            print(
                f"✓ {thread_count} threads: {metrics.execution_time:.2f}s, CPU: {metrics.cpu_percent:.1f}%"
            )
        ...
        if 1 in results and 4 in results:
            ...
            assert (
                efficiency > 0.2
            ), f"Parallel processing efficiency too low: {efficiency:.2f}"

            print(
                f"✓ Parallel speedup: {speedup:.2f}x with 4 threads (efficiency: {efficiency:.2f})"
            )
    ...
    def test_cache_effectiveness(self):
        ...
        io_reduction = (
            cold_metrics.io_read_mb - warm_metrics.io_read_mb
        ) / cold_metrics.io_read_mb
        assert (
            io_reduction > 0.5
        ), f"Cache I/O reduction insufficient: {io_reduction:.2f}"

        print(
            f"✓ Cache effectiveness: {speedup:.2f}x speedup, {io_reduction:.1%} I/O reduction"
        )
    ...
    def test_full_pipeline_performance(self):
        ...
        with performance_monitor(operation_name, dataset_size_mb):
            ...
            final_data, quality_report = quality_controller.assess_and_improve_data(
                processed_data
            )
        ...
        assert (
            time_ratio < 1.5
        ), f"Full pipeline too slow: {metrics.execution_time:.2f}s vs {baseline['time']}s baseline"
        assert (
            memory_ratio < 1.5
        ), f"Full pipeline uses too much memory: {metrics.peak_memory_mb:.1f}MB vs {baseline['memory']}MB baseline"

        print(
            f"✓ Full pipeline: {metrics.execution_time:.2f}s, {metrics.peak_memory_mb:.1f}MB, {metrics.throughput_mbps:.1f}MB/s"
        )
...
class TestScalabilityValidation:
    ...
    def test_memory_scalability_large_datasets(self):
        ...
        for size in ["small", "medium", "large"]:
            dataset_key = (
                f"performance_{size}" if size != "small" else "integration_small"
            )
        ...
        for size_name, dataset_key in available_sizes:
            ...
            with performance_monitor(operation_name, dataset_size_mb):
                memory_config = MemoryConfig(
                    strategy=MemoryStrategy.BALANCED, enable_memory_mapping=True
                )
                memory_manager = MemoryManager(memory_config)
            ...
            print(
                f"✓ {size_name}: {dataset_size_mb:.1f}MB dataset → {metrics.peak_memory_mb:.1f}MB memory (efficiency: {memory_efficiency:.2f})"
            )
        ...
        if len(memory_results) >= 2:
            sizes = sorted(
                memory_results.keys(),
                key=lambda x: memory_results[x]["dataset_size_mb"],
            )
            ...
            assert (
                efficiency_degradation < 3.0
            ), f"Memory efficiency degrades too much: {efficiency_degradation:.2f}x"

            print(
                f"✓ Memory scalability: Efficiency degradation {efficiency_degradation:.2f}x from {sizes[0]} to {sizes[-1]}"
            )
    ...
    def test_processing_time_scalability(self):
        ...
        complexity_configs = [
            ...
            (
                ...
                {
                    "filtering": FilterConfig(
                        quality_threshold=0.9, enable_advanced_quality=True
                    ),
                    "preprocessing": PreprocessingConfig(
                        pipeline_stages=[
                            "diagonal_correction",
        ...
        if len(timing_results) >= 2:
            ...
            complex_time = timing_results.get(
                "complex", timing_results[max(timing_results.keys())]
            )

            if simple_time > 0:
                complexity_overhead = complex_time / simple_time
                assert (
                    complexity_overhead < 5.0
                ), f"Complexity overhead too high: {complexity_overhead:.2f}x"

                print(
                    f"✓ Complexity scaling: {complexity_overhead:.2f}x overhead for complex processing"
                )
    ...
    def test_concurrent_load_scalability(self):
        ...
        for num_concurrent in concurrent_levels:
            ...
            with performance_monitor(operation_name, 0):
                with concurrent.futures.ThreadPoolExecutor(
                    ...
                    futures = [
                        executor.submit(load_and_process) for _ in range(num_concurrent)
                    ]
            ...
            print(
                f"✓ {num_concurrent} concurrent: {metrics.execution_time:.2f}s, {metrics.peak_memory_mb:.1f}MB"
            )
        ...
        if 1 in results and 4 in results:
            ...
            assert (
                time_degradation < 2.0
            ), f"Concurrent performance degrades too much: {time_degradation:.2f}x"
            assert (
                memory_increase < 4.0
            ), f"Concurrent memory usage too high: {memory_increase:.2f}x"

            print(
                f"✓ Concurrent scaling: {time_degradation:.2f}x time, {memory_increase:.2f}x memory for 4x concurrency"
            )
...
class TestPerformanceRegression:
    ...
    def save_baselines(cls):
        ...
        if hasattr(performance_monitor, "results"):
            ...
            for operation_name, metrics in performance_monitor.results.items():
                ...
                if (
                    not current_baseline
                    or metrics.execution_time
                    < current_baseline.get("time", float("inf"))
                ):
    ...
    def test_loading_regression(self):
        ...
        if baseline:
            ...
            assert (
                time_ratio < self.PERFORMANCE_REGRESSION_TOLERANCE
            ), f"Loading time regression: {time_ratio:.2f}x slower than baseline"
            assert (
                memory_ratio < self.PERFORMANCE_REGRESSION_TOLERANCE
            ), f"Loading memory regression: {memory_ratio:.2f}x more memory than baseline"

            print(
                f"✓ Loading regression check: {time_ratio:.2f}x time, {memory_ratio:.2f}x memory vs baseline"
            )
        else:
    ...
    def test_full_pipeline_regression(self):
        ...
        if baseline:
            ...
            print(
                f"✓ Pipeline regression check: {time_ratio:.2f}x time, {memory_ratio:.2f}x memory vs baseline"
            )
        else:
...
def test_performance_reporting():
    ...
    sorted_by_throughput = sorted(
        results.items(), key=lambda x: x[1].throughput_mbps, reverse=True
    )
    ...
    for operation, metrics in results.items():
        if metrics.dataset_size_mb > 0:
            ...
            print(
                f"  {operation}: {efficiency:.2f} (dataset: {metrics.dataset_size_mb:.1f}MB, peak: {metrics.peak_memory_mb:.1f}MB)"
            )
    ...
    for operation, metrics in results.items():
        if metrics.execution_time > 30.0:  # Slow operations
            performance_issues.append(
                f"{operation} is slow: {metrics.execution_time:.2f}s"
            )
        if metrics.peak_memory_mb > 2000:  # High memory usage
            performance_issues.append(
                f"{operation} uses excessive memory: {metrics.peak_memory_mb:.1f}MB"
            )
        if (
            metrics.dataset_size_mb > 0 and metrics.throughput_mbps < 5
        ):  # Low throughput
            performance_issues.append(
                f"{operation} has low throughput: {metrics.throughput_mbps:.1f}MB/s"
            )
