...
try:
    ...
    from homodyne.data.performance_engine import (PerformanceConfig,
                                                  PerformanceEngine)
                    ...
                    if key == "analysis" and "analysis" in src:
                        ...
                        if "correlation" in src["analysis"]:
                            ...
                            if "g2" in src["analysis/correlation"]:
                                ...
                                # Add NaN values
                                nan_mask = (
                                    np.random.random(g2_data.shape) < 0.05
                                )  # 5% NaN
                                g2_data[nan_mask] = np.nan
                                # Add Inf values
                                inf_mask = (
                                    np.random.random(g2_data.shape) < 0.02
                                )  # 2% Inf
                            ...
                            for corr_key in src["analysis/correlation"].keys():
                                if corr_key != "g2":
                                    src.copy(
                                        f"analysis/correlation/{corr_key}", corr_group
                                    )
                    else:
                        src.copy(key, dst)
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            try:
                ...
                if data is None:
                    ...
                    assert any(
                        "missing" in str(warning.message).lower() for warning in w
                    )
            except:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            if data is not None and "g2" in data:
                ...
                if has_nan or has_inf:
                    ...
                    assert any(
                        "nan" in msg or "inf" in msg or "invalid" in msg
            ...
            if data is not None:
                ...
                improved_data, quality_report = (
                    quality_controller.assess_and_improve_data(data)
                )

                # Quality should be flagged as poor
                assert (
                    quality_report["overall_score"] < 0.7
                ), "Contaminated data should have poor quality score"

                # Auto-repair should attempt to fix issues
                if qc_config.enable_auto_repair:
                    assert (
                        quality_report.get("repairs_applied", 0) > 0
                    ), "Should attempt repairs on contaminated data"
    ...
    def test_partial_corruption_recovery(self):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            qc_config = QualityControlConfig(
                enable_auto_repair=True, enable_progressive=True
            )
            quality_controller = QualityController(qc_config)

            improved_data, quality_report = quality_controller.assess_and_improve_data(
                data
            )
...
class TestResourceConstraintAdaptation:
    ...
    def test_low_memory_adaptation(self):
        ...
        with memory_manager.memory_context():
            ...
            assert (
                loader.should_use_memory_mapping()
            ), "Should use memory mapping under memory constraints"

            data = loader.load_data()
            assert (
                data is not None
            ), "Should be able to load data under memory constraints"
    ...
    def test_disk_space_constraint_handling(self):
        ...
        with performance_engine.optimized_context():
            ...
            for i in range(3):
                ...
                cache_size = sum(
                    f.stat().st_size
                    for f in limited_cache_dir.rglob("*")
                    if f.is_file()
                )
                ...
                assert (
                    cache_size_mb < 100
                ), f"Cache size ({cache_size_mb:.1f}MB) exceeded reasonable limit"
    ...
    def test_cpu_constraint_adaptation(self):
        ...
        with performance_engine.optimized_context():
            ...
            ), "Preprocessing should work under CPU constraints"
    ...
    def test_memory_pressure_response(self):
        ...
        with memory_pressure_simulation(target_usage_mb=500):
            ...
            with memory_manager.memory_context():
                ...
                assert (
                    memory_manager.is_under_memory_pressure()
                ), "Should detect simulated memory pressure"
...
class TestConcurrentAccessSafety:
    ...
    def test_concurrent_data_loading(self):
        ...
        def load_data_worker(worker_id: int):
            ...
            try:
                ...
                assert np.all(
                    g2_data >= 1.0
                ), f"Worker {worker_id}: g2 constraint violation"
                assert np.all(
                    np.isfinite(g2_data)
                ), f"Worker {worker_id}: non-finite values"
        ...
        assert all(
            shape == reference_shape for shape in shapes
        ), "Inconsistent data shapes from concurrent loading"
    ...
    def test_concurrent_processing_safety(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            futures = [
                executor.submit(process_data_worker, i) for i in range(num_workers)
            ]
            concurrent.futures.wait(futures)
    ...
    def test_thread_local_storage_isolation(self):
        ...
        def worker_with_local_state(
            worker_id: int, config_modification: Dict[str, Any]
        ):
            ...
            try:
                ...
                thread_results[worker_id] = {
                    ...
                    "data_shape": (
                        filtered_data["g2"].shape
                        if filtered_data and "g2" in filtered_data
                        else None
                    ),
                    "success": True,
                }
                ...
                thread_results[worker_id] = {"error": str(e), "success": False}
        ...
        for worker_id, result in thread_results.items():
            assert result[
                "success"
            ], f"Worker {worker_id} failed: {result.get('error')}"
            assert (
                result["config"] == worker_configs[worker_id]
            ), f"Configuration not isolated for worker {worker_id}"

        print(
            f"✓ Thread-local storage isolation: {len(worker_configs)} threads with different configs"
        )
    ...
    def test_resource_contention_handling(self):
        ...
        assert (
            len(successful_workers) >= num_workers // 2
        ), "Too many workers failed under resource contention"

        if len(failed_workers) > 0:
            print(
                f"⚠ {len(failed_workers)} workers failed under resource contention (acceptable)"
            )
        ...
        if len(completion_times_only) > 1:
            ...
            assert (
                cv < 2.0
            ), f"Excessive completion time variation under contention: CV={cv:.2f}"

        print(
            f"✓ Resource contention handling: {len(successful_workers)}/{num_workers} workers succeeded"
        )
...
class TestConfigurationRobustness:
    ...
    def test_invalid_configuration_handling(self):
        ...
        invalid_configs = [
            # Invalid q-range
            {
                "data_loading": {"filtering": {"q_range": {"min": 1e-2, "max": 1e-4}}}
            },  # min > max
            # Invalid thresholds
            ...
            {
                "data_loading": {"performance": {"max_threads": 1000}}
            },  # Unreasonably high
        ...
        try:
            ...
            try:
                ...
                if data is not None:
                    ...
                    assert "g2" in data
                    print(
                        f"✓ Loaded data successfully with {len(open_files)} open file descriptors"
                    )
                else:
                    print(
                        "⚠ Data loading failed under file descriptor pressure (acceptable)"
                    )
        ...
        with memory_manager.memory_context():
            ...
            try:
                ...
                if data is not None:
                    ...
                    assert (
                        memory_used < 2 * memory_limit
                    ), f"Memory usage ({memory_used / 1024**2:.1f}MB) exceeded limit significantly"

                    print(
                        f"✓ Loaded data within memory constraints: {memory_used / 1024**2:.1f}MB used"
                    )
                else:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                try:
                    ...
                    if data is not None:
                        ...
                        print(
                            "⚠ Operation timed out (expected with aggressive timeout)"
                        )
