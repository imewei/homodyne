...
def test_gradient_computations():
    ...
    try:
        ...
        def g2_scalar(params):
            result = jax_backend.compute_g2_scaled(params, test_t1, test_t2, phi, test_q, L, contrast, offset)
            return result[0]  # Return first element as scalar
        
        def chi2_scalar(params):
            data = np.array([1.5])  # Single data point
            sigma = np.array([0.01])
            return jax_backend.compute_chi_squared(params, data, sigma, test_t1, test_t2, phi, test_q, L, contrast, offset)
