...
def test_with_jax():
    ...
    try:
        ...
        from homodyne.optimization import fit_homodyne_vi, fit_homodyne_mcmc, VI_FALLBACK_MODE, MCMC_FALLBACK_MODE
        
        print(f"VI fallback mode: {VI_FALLBACK_MODE}")
        print(f"MCMC fallback mode: {MCMC_FALLBACK_MODE}")
        
        # Generate test data
        data, sigma, t1, t2, phi, q, L, true_params, true_contrast, true_offset = generate_test_data()
        ...
        mcmc_result = fit_homodyne_mcmc(data, sigma, t1, t2, phi, q, L,
                                       analysis_mode="static_isotropic", 
                                       n_samples=50, n_warmup=50, n_chains=2,
                                       vi_init=vi_result)
...
def test_without_jax():
    ...
    try:
        ...
        vi_result = fit_homodyne_vi(data, sigma, t1, t2, phi, q, L,
                                   analysis_mode="static_isotropic",
                                   n_iterations=50)  # Fewer iterations for fallback
        vi_time = time.time() - start_time
        ...
        mcmc_result = fit_homodyne_mcmc(data, sigma, t1, t2, phi, q, L,
                                       analysis_mode="static_isotropic",
                                       n_samples=25, n_warmup=25, n_chains=2,  # Very small for fallback
                                       vi_init=vi_result)
        mcmc_time = time.time() - start_time
...
def validate_results(jax_vi_result, jax_mcmc_result, fallback_vi_result, fallback_mcmc_result, true_params):
    ...
    if jax_vi_result and fallback_vi_result:
        ...
        param_errors = np.abs(fallback_vi_result.mean_params - true_params) / np.abs(true_params)
        validation_results['vi_params_reasonable'] = np.all(param_errors < 1.0)  # Within 100%
        
        # Check if VI results are similar between JAX and fallback
        if jax_vi_result.backend != fallback_vi_result.backend:
            vi_diff = np.abs(jax_vi_result.mean_params - fallback_vi_result.mean_params) / np.abs(jax_vi_result.mean_params)
        ...
        print(f"VI JAX vs fallback agreement: {validation_results['vi_fallback_close']}")
    
    if jax_mcmc_result and fallback_mcmc_result:
        # Check if fallback MCMC parameters are reasonable
        param_errors = np.abs(fallback_mcmc_result.mean_params - true_params) / np.abs(true_params)
        validation_results['mcmc_params_reasonable'] = np.all(param_errors < 1.0)  # Within 100%
        
        # Check if MCMC results are similar between JAX and fallback
        if jax_mcmc_result.backend != fallback_mcmc_result.backend:
            mcmc_diff = np.abs(jax_mcmc_result.mean_params - fallback_mcmc_result.mean_params) / np.abs(jax_mcmc_result.mean_params)
            validation_results['mcmc_fallback_close'] = np.all(mcmc_diff < tolerance)
        
        print(f"MCMC parameter accuracy: {validation_results['mcmc_params_reasonable']}")
        print(f"MCMC JAX vs fallback agreement: {validation_results['mcmc_fallback_close']}")
...
def main():
    ...
    if jax_success and fallback_success:
        validation_results = validate_results(
            jax_vi_result, jax_mcmc_result, 
            fallback_vi_result, fallback_mcmc_result,
            true_params
        )
        ...
        print(f"Parameter accuracy: {'PASSED' if validation_results.get('vi_params_reasonable', False) else 'FAILED'}")
        print(f"Fallback agreement: {'PASSED' if validation_results.get('vi_fallback_close', False) else 'MODERATE'}")
        
        overall_success = (jax_success and fallback_success and 
                          validation_results.get('vi_params_reasonable', False))
