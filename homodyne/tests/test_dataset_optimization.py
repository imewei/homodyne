...
try:
    from homodyne.data.optimization import (DatasetInfo, DatasetOptimizer,
                                            ...
                                            optimize_for_method)
    from homodyne.optimization import fit_mcmc_jax, fit_vi_jax
...
@pytest.mark.skipif(
    not OPTIMIZATION_AVAILABLE, reason="Dataset optimization not available"
)
class TestDatasetOptimizer:
    ...
    def test_dataset_analyzer_small(self):
        ...
        assert (
            dataset_info.recommended_chunk_size == dataset_info.size
        )  # No chunking needed
    ...
    def test_chunked_iterator(self):
        ...
        chunk_size = 1000
        chunks = list(
            self.optimizer.create_chunked_iterator(
