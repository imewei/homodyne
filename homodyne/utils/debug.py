...
def debug_calls(logger: Optional[logging.Logger] = None,
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if track_memory and call_info.memory_delta is not None:
                    exit_msg += f" [Memory Δ: {call_info.memory_delta / 1024 / 1024:.2f}MB]"
                ...
                if track_memory and call_info.memory_delta is not None:
                    error_msg += f" [Memory Δ: {call_info.memory_delta / 1024 / 1024:.2f}MB]"
...
def profile_memory(logger: Optional[logging.Logger] = None,
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.error(f"Memory profiling interrupted by exception in {func_name}: {e}")
                raise
    ...
    def end_profile(self, name: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'efficiency': (end_cpu_time - profile['start_cpu_time']) / (end_time - profile['start_time'])
            }
...
def profile_performance(logger: Optional[logging.Logger] = None,
                       detailed: bool = False):
    """
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.info(f"Performance {func_name}: Wall={wall_time:.3f}s, CPU={cpu_time:.3f}s")
                ...
                end_time = time.perf_counter()
                wall_time = end_time - start_time
                logger.error(f"Performance {func_name} failed after {wall_time:.3f}s: {e}")
...
def debug_context(operation_name: str,
    ...
    try:
        ...
        if track_performance:
            ...
            if profile_data:
                logger.debug(f"Performance profile for {operation_name}: {profile_data}")
        ...
        error_msg = f"DEBUG CONTEXT ERROR: {operation_name} [Duration: {duration:.3f}s]: {e}"
