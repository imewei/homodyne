...
from .debug import CallTracker, MemoryTracker
from .logging import get_logger, log_operation, log_performance
    ...
    def record_compilation(self, event: JITCompilationEvent):
        ...
        with self._lock:
            ...
            if len(self._compilation_times[event.function_name]) > 50:
                self._compilation_times[event.function_name] = self._compilation_times[
                    event.function_name
                ][-25:]
...
class JAXMemoryTracker:
    ...
    def get_peak_usage(self) -> Dict[str, JAXMemorySnapshot]:
        ...
        with self._lock:
            ...
            for snapshot in self._snapshots:
                ...
                if (
                    device not in peak_by_device
                    or snapshot.allocated_bytes > peak_by_device[device].allocated_bytes
                ):
    ...
    def get_usage_timeline(
        self, device: Optional[str] = None, window_size: int = 100
    ) -> List[JAXMemorySnapshot]:
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if track_memory:
                ...
                if current_usage:
                    memory_before = sum(
                        s.allocated_bytes for s in current_usage.values()
                    )
            ...
            try:
                ...
                compilation_time = time.perf_counter() - start_time
                logger.error(
                    f"JIT compilation failed for {func_name} after {compilation_time:.3f}s: {e}"
                )
                raise
...
def jax_operation_context(
    ...
    try:
        ...
        success_msg = (
            f"JAX operation completed: {operation_name} [{duration:.3f}s]{device_info}"
        )
                    ...
                    try:
                        ...
                        if hasattr(result, "__iter__"):
                            ...
                            if norm < norm_threshold:
                                ...
                                op_logger.debug(
                                    f"Gradient norm for {func_name}: {norm:.2e}"
                                )

                    except Exception as e:
                        op_logger.debug(
                            f"Could not analyze gradients for {func_name}: {e}"
                        )
