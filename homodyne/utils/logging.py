...
class HomodyneFormatter(logging.Formatter):
    ...
    def _error_format(self):
        return (
            "%(asctime)s | ERROR | %(name)s.%(function_name)s:%(lineno)d | %(message)s"
        )
    ...
    def _info_format(self):
        if self.include_module and self.include_function:
            return (
                "%(asctime)s | INFO  | %(module_name)s.%(function_name)s | %(message)s"
            )
    ...
    def _debug_format(self):
        return (
            "%(asctime)s | DEBUG | %(name)s.%(function_name)s:%(lineno)d | %(message)s"
        )
            ...
            try:
                ...
                while frame:
                    ...
                    if frame and frame.f_code.co_filename != __file__:
                        record.caller_module = os.path.basename(
...
class LoggerManager:
    ...
    def configure_from_json(self, config: Dict[str, Any]):
        ...
        """
        # Allow reconfiguration if config file specifies file logging
        # This supports the use case where CLI sets up basic logging
        # but config file wants to add file logging
        logging_config = config.get("logging", {})
        file_config = logging_config.get("file", {})
        file_enabled = file_config.get("enabled", False)
        
        if self._configured and not file_enabled:
            return
        
        # If already configured but file logging is requested, just add file handler
        if self._configured and file_enabled:
            self._add_file_handler_from_config(logging_config)
            return
        ...
        if console_enabled:
            ...
            if console_colors and console_format == "detailed":
                console_formatter = ConsoleFormatter(
                    include_function=True, include_module=True
                )
            elif console_format == "detailed":
                console_formatter = HomodyneFormatter(
                    include_function=True, include_module=True
                )
            else:
                console_formatter = HomodyneFormatter(
                    include_function=False, include_module=True
                )
        ...
        if file_enabled:
            ...
            file_formatter = HomodyneFormatter(
                include_function=True, include_module=True
            )
    ...
    def _add_file_handler_from_config(self, logging_config: Dict[str, Any]):
        """Add file handler to existing logging configuration."""
        file_config = logging_config.get("file", {})
        file_level = self._parse_log_level(file_config.get("level", "DEBUG"))
        log_path = file_config.get("path", "~/.homodyne/logs/")
        log_filename = file_config.get("filename", "homodyne.log")
        ...
        # Check if file handler already exists with same path
        log_file = log_dir / log_filename
        for handler in root_logger.handlers:
            if isinstance(handler, logging.handlers.RotatingFileHandler):
                if Path(handler.baseFilename) == log_file.resolve():
                    # File handler already exists, don't add duplicate
                    return
        ...
        print(f"📝 Config file logging enabled: {log_file}")
...
def configure_testing():
    ...
    configure_logging(level=logging.WARNING, console_output=False, file_output=False)
...
def load_and_apply_logging_config(
...
def validate_logging_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if file_config.get("enabled", True):
        ...
        if not isinstance(backup_count, int) or backup_count < 0:
            errors.append(
                f"Invalid backup_count: {backup_count} (must be non-negative integer)"
            )
    ...
    if perf_config.get("enabled", True):
        ...
        if not isinstance(threshold, (int, float)) or threshold < 0:
            warnings.append(
                f"Invalid performance threshold: {threshold} (should be non-negative number)"
            )
...
def get_effective_logging_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    effective_config = {
        ...
        "console": {
            ...
            "level": logging_config.get("console", {}).get(
                "level", logging_config.get("level", "INFO")
            ),
        ...
        "performance": {
            ...
            "filename": logging_config.get("performance", {}).get(
                "filename", "performance.log"
            ),
            "threshold_seconds": logging_config.get("performance", {}).get(
        ...
        "debug": {
            ...
            "profile_performance": logging_config.get("debug", {}).get(
