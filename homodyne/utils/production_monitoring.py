...
class MetricsCollector:
    ...
    def __init__(self, logger: Optional = None):
        ...
        self._aggregation_thread = threading.Thread(
            target=self._background_aggregation, daemon=True
        )
...
class HealthChecker:
    ...
    def register_check(
        self, check_name: str, check_function: Callable[[], HealthCheckResult]
    ):
    ...
    def _check_memory_usage(self) -> HealthCheckResult:
        ...
        try:
            ...
            message = (
                f"Process memory: {memory_mb:.0f}MB ({memory_percent:.1f}% of system)"
            )
        ...
        try:
            ...
            if not log_dir.exists():
                return HealthCheckResult(
                    ...
                    execution_time=0.0,
                    recommendations=[
                        "Create log directory or check logging configuration"
                    ],
                )
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "individual_results": {
                name: asdict(result) for name, result in latest_results.items()
            },
        }
...
class AlertManager:
    ...
    def raise_alert(
        ...
        """Raise a new alert."""
        alert = Alert(
            level=level, title=title, message=message, source=source, tags=tags or {}
        )
        ...
        self.logger.log(
            log_level,
            f"ALERT [{level.value.upper()}] {title}: {message} (source: {source})",
        )
    ...
    def _send_notifications(self, alert: Alert):
        ...
        # Email notification
        if (
            self.notification_settings.get("email_enabled")
            and alert.level.value
            >= self.notification_settings["min_level_for_email"].value
        ):
            threading.Thread(
                target=self._send_email_notification, args=(alert,), daemon=True
            ).start()

        # Webhook notification
        if (
            self.notification_settings.get("webhook_enabled")
            and alert.level.value
            >= self.notification_settings["min_level_for_webhook"].value
        ):
            threading.Thread(
                target=self._send_webhook_notification, args=(alert,), daemon=True
            ).start()
    ...
    def _send_email_notification(self, alert: Alert):
        ...
        try:
            ...
            msg["Subject"] = (
                f"[Homodyne Alert - {alert.level.value.upper()}] {alert.title}"
            )
...
"""
            ...
            text = msg.as_string()
            server.sendmail(
                settings["smtp_username"], settings["email_recipients"], text
            )
            server.quit()
    ...
    def get_alert_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "oldest_active_alert_age": (
                    max([a.age_seconds for a in active_alerts]) if active_alerts else 0
                ),
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                _metrics_collector.record_timer(
                    f"operation.{operation_name}.failure_duration", duration
                )
...
def production_monitoring_context(
    ...
    try:
        ...
        logger.info(
            f"Production operation completed: {operation_name} [{duration:.2f}s]"
        )
        ...
        _alert_manager.raise_alert(
            ...
            title=(
                ...
                if critical
                else f"Operation Failed: {operation_name}"
            ),
            message=f"Operation failed after {duration:.2f}s: {str(e)}",
            source="production_monitoring",
            tags={"operation": operation_name, "error_type": type(e).__name__},
        ...
        logger.error(
            f"Production operation failed: {operation_name} [{duration:.2f}s]: {e}"
        )
        raise
...
def get_production_monitoring_stats() -> Dict[str, Any]:
    ...
    return {
        ...
        "active_alerts": [
            asdict(alert) for alert in _alert_manager.get_active_alerts()
        ],
        "system_capabilities": {
