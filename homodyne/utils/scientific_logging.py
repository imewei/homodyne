...
class XPCSDataValidator:
    ...
    def validate_physics_parameter(
    ...
    def validate_correlation_data(
        self, correlation_data, q_vectors: Optional[np.ndarray] = None
    ) -> List[str]:
        ...
        if HAS_NUMPY:
            try:
                ...
                if variance < self.stability_thresholds["min_data_variance"]:
                    warnings.append(
                        f"Very low data variance ({variance:.2e}) may indicate processing issues"
                    )
            except:
        ...
        if HAS_NUMPY:
            try:
                ...
                if dynamic_range > self.stability_thresholds["max_parameter_ratio"]:
                    warnings.append(
                        f"Large dynamic range ({dynamic_range:.1e}) may cause numerical issues"
                    )
            except:
        ...
        if q_vectors is not None and hasattr(q_vectors, "shape"):
            if len(q_vectors.shape) > 0 and correlation_data.shape[-1] != len(
                q_vectors
            ):
                warnings.append(
                    f"Q-vector count ({len(q_vectors)}) doesn't match correlation data shape ({correlation_data.shape})"
                )
    ...
    def record_iteration(self, snapshot: FittingProgressSnapshot):
        ...
        with self._lock:
            ...
            if snapshot.gradient_norm:
                self._convergence_history["gradient_norm"].append(
                    snapshot.gradient_norm
                )

    def check_convergence(
        self, window_size: int = 10, tolerance: float = 1e-6
    ) -> Tuple[bool, str]:
        ...
        with self._lock:
            ...
            if loss_std / max(abs(loss_mean), 1e-15) < tolerance:
                return (
                    True,
                    f"Loss converged (std/mean = {loss_std / abs(loss_mean):.2e})",
                )
            ...
            if self._convergence_history["gradient_norm"]:
                ...
                if recent_grads[-1] < tolerance:
                    return (
                        True,
                        f"Gradient norm below threshold ({recent_grads[-1]:.2e})",
                    )

            return (
                False,
                f"Not converged (loss std/mean = {loss_std / abs(loss_mean):.2e})",
            )
    ...
    def get_progress_summary(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            summary = {
                ...
                "improvement": (initial.loss_value - latest.loss_value)
                / initial.loss_value,
...
@contextmanager
def xpcs_data_loading_context(
    filepath: Union[str, Path], expected_format: str = "auto", logger: Optional = None
):
    ...
    data_info = XPCSDataInfo(filepath=str(filepath), file_format=expected_format)
    ...
    try:
        ...
        if data_info.preprocessing_applied:
            logger.info(
                f"Preprocessing applied: {', '.join(data_info.preprocessing_applied)}"
            )
...
@contextmanager
def correlation_computation_context(
    method: str,
    ...
    metrics = CorrelationComputationMetrics(
        computation_method=method, input_data_shape=input_shape
    )
    ...
    if HAS_JAX and method in ["jax_vectorized", "jax_chunked"]:
        with jax_operation_context(f"correlation_{method}", logger, track_memory):
            try:
                ...
                metrics.computation_time = time.perf_counter() - start_time
                logger.error(
                    f"Correlation computation failed after {metrics.computation_time:.2f}s: {e}"
                )
                raise
    else:
        ...
        try:
            ...
            metrics.computation_time = time.perf_counter() - start_time
            logger.info(
                f"Correlation computation completed in {metrics.computation_time:.2f}s"
            )
                ...
                if isinstance(result, dict):
                    ...
                    for param_name, value in result.items():
                        if isinstance(value, (int, float)):
                            validation = _xpcs_validator.validate_physics_parameter(
                            ...
                            if not validation.is_valid:
                                logger.warning(
                                    f"Parameter validation failed: {validation.warning_message}"
                                )
                    ...
                    if total_count > 0:
                        logger.info(
                            f"Physics validation: {valid_count}/{total_count} parameters valid"
                        )
                ...
                for i, item in enumerate(result):
                    if hasattr(item, "shape"):
                        ...
                        if HAS_NUMPY:
                            ...
                            condition_number = (
                                np.linalg.cond(item) if item.ndim == 2 else None
                            )
                            if (
                                condition_number
                                and condition_number
                                > _xpcs_validator.stability_thresholds[
                                    "max_condition_number"
                                ]
                            ):
                                logger.warning(
                                    f"High condition number in {array_name}[{i}]: {condition_number:.2e}"
                                )
