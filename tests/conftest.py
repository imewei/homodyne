...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if hasattr(item, 'fixturenames') and any('jax' in name for name in item.fixturenames):
...
class TestUtils:
    ...
    def assert_array_properties(arr, expected_shape=None, dtype=None):
        ...
        if expected_shape is not None:
            assert arr.shape == expected_shape, f"Shape mismatch: {arr.shape} != {expected_shape}"
