...
# ==============================================================================
# ArviZ Integration Workflow Tests (Added v2.4.1+)
# ==============================================================================
...
class TestArviZWorkflows:
    ...
    def mock_mcmc_result_with_samples(self):
        ...
        # Generate realistic chain samples
        np.random.seed(42)

        # Per-angle contrast (3 angles)
        contrast_samples = np.random.normal(0.5, 0.02, (n_chains, n_samples, 3))
        # Per-angle offset (3 angles)
        offset_samples = np.random.normal(1.0, 0.01, (n_chains, n_samples, 3))
        # Physical params: D0, alpha, D_offset
        d0_samples = np.random.lognormal(np.log(1000), 0.1, (n_chains, n_samples, 1))
        alpha_samples = np.random.normal(0.5, 0.05, (n_chains, n_samples, 1))
        d_offset_samples = np.random.lognormal(np.log(10), 0.2, (n_chains, n_samples, 1))

        # Combine in NumPyro ordering
        ...
        return MCMCResult(
            mean_params=np.mean(samples_flat, axis=0),
            mean_contrast=0.5,
            mean_offset=1.0,
            std_params=np.std(samples_flat, axis=0),
            samples_params=samples_flat,
            ci_95_lower=np.percentile(samples_flat, 2.5, axis=0),
            ci_95_upper=np.percentile(samples_flat, 97.5, axis=0),
            param_names=param_names,
            n_chains=n_chains,
            n_samples=n_samples,
            analysis_mode="static",
            r_hat={"D0": 1.02, "alpha": 1.01, "D_offset": 1.03},
            effective_sample_size={"D0": 180, "alpha": 195, "D_offset": 175},
        )

    def test_mcmc_result_to_arviz_conversion(self, mock_mcmc_result_with_samples):
        """Test complete MCMCResult → ArviZ InferenceData workflow."""
        result = mock_mcmc_result_with_samples

        # Convert to ArviZ
        idata = result.to_arviz()

        # Verify InferenceData structure
        assert isinstance(idata, az.InferenceData)
        assert "posterior" in idata.groups()

        # Verify posterior contains expected parameters
        posterior = idata.posterior
        assert hasattr(posterior, "D0")
        assert hasattr(posterior, "alpha")
        assert hasattr(posterior, "D_offset")

        # Verify dimensions (may be flattened depending on implementation)
        assert posterior.D0.dims == ("chain", "draw")
        # Note: to_arviz may reshape samples differently
        total_samples = posterior.D0.shape[0] * posterior.D0.shape[1]
        assert total_samples == result.n_chains * result.n_samples

    def test_arviz_summary_statistics(self, mock_mcmc_result_with_samples):
        """Test ArviZ summary statistics computation from MCMCResult."""
        result = mock_mcmc_result_with_samples

        # Convert and compute summary
        idata = result.to_arviz()
        ...
        assert "hdi_3%" in summary.columns or "hdi_2.5%" in summary.columns

        # Verify parameter count
        assert len(summary) == 3  # D0, alpha, D_offset

        # Verify values are finite
        assert np.all(np.isfinite(summary["mean"].values))
        assert np.all(np.isfinite(summary["sd"].values))

    def test_arviz_diagnostics_workflow(self, mock_mcmc_result_with_samples):
        """Test ArviZ diagnostics (R-hat, ESS) computation workflow."""
        result = mock_mcmc_result_with_samples

        idata = result.to_arviz()

        # Compute R-hat (may return NaN if single chain or insufficient samples)
        rhat = az.rhat(idata, var_names=["D0", "alpha", "D_offset"])
        assert "D0" in rhat.data_vars
        rhat_d0 = float(rhat["D0"].values)
        # R-hat may be NaN with 1 chain; if finite, should be reasonable
        if np.isfinite(rhat_d0):
            assert rhat_d0 < 1.5  # Reasonable convergence

        # Compute ESS
        ess = az.ess(idata, var_names=["D0", "alpha", "D_offset"])
        assert "D0" in ess.data_vars
        ess_d0 = float(ess["D0"].values)
        if np.isfinite(ess_d0):
            assert ess_d0 > 10  # At least some effective samples

    def test_arviz_plotting_workflow(self, mock_mcmc_result_with_samples, tmp_path):
        """Test ArviZ plotting functions workflow."""
        import matplotlib
        ...
        plt.savefig(tmp_path / "trace.png")
        plt.close()

        assert (tmp_path / "trace.png").exists()

        # Test posterior plot
        az.plot_posterior(idata, var_names=["D0", "alpha", "D_offset"])
        plt.savefig(tmp_path / "posterior.png")
        plt.close()

        assert (tmp_path / "posterior.png").exists()

    def test_homodyne_plot_functions_with_arviz(self, mock_mcmc_result_with_samples, tmp_path):
        """Test homodyne.viz plotting functions with ArviZ integration."""
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        ...
        # Test homodyne trace plots
        fig = plot_trace_plots(result, save_path=str(tmp_path / "homodyne_trace.png"))
        assert fig is not None
        plt.close(fig)

        # Test homodyne ArviZ posterior plots
        fig = plot_arviz_posterior(result, save_path=str(tmp_path / "homodyne_posterior.png"))
        assert fig is not None
        plt.close(fig)

    def test_nlsq_to_mcmc_to_arviz_pipeline(self):
        """Test complete NLSQ → MCMC → ArviZ workflow.

        This test simulates the full user workflow:
        1. User runs NLSQ to get point estimates
        2. User copies NLSQ results to config
        ...
        # Step 2: Config with NLSQ results
        config = {
            "analysis_mode": "static",
            "initial_parameters": {
                "parameter_names": ["D0", "alpha", "D_offset"],
                "values": [nlsq_best_fit["D0"], nlsq_best_fit["alpha"], nlsq_best_fit["D_offset"]],
            },
        }

        # Step 3: Simulated MCMC result (would come from fit_mcmc_jax)
        np.random.seed(42)
        n_samples = 200
        samples = np.column_stack([
            np.random.normal(nlsq_best_fit["D0"], 50, n_samples),
            np.random.normal(nlsq_best_fit["alpha"], 0.02, n_samples),
            np.random.normal(nlsq_best_fit["D_offset"], 1.0, n_samples),
        ])
        ...
        # Step 4: Export to ArviZ
        idata = mcmc_result.to_arviz()

        # Verify the pipeline worked
        assert isinstance(idata, az.InferenceData)

        # Verify MCMC samples are close to NLSQ point estimates
        d0_mean = float(idata.posterior["D0"].mean())
        assert abs(d0_mean - nlsq_best_fit["D0"]) < 100  # Within 100 of NLSQ

        # Verify uncertainty quantification
        d0_std = float(idata.posterior["D0"].std())
        assert d0_std > 0  # Has uncertainty estimate
        assert d0_std < nlsq_best_fit["D0"]  # Reasonable uncertainty
    ...
    def test_cmc_result_to_arviz_with_diagnostics(self):
        ...
        samples = np.random.normal(1.0, 0.1, (n_samples, n_params))

        # Create CMC result with per-shard diagnostics
        per_shard_diagnostics = [
            {"shard_id": 0, "r_hat": {"D0": 1.01}, "ess": {"D0": 95}, "converged": True},
            {"shard_id": 1, "r_hat": {"D0": 1.02}, "ess": {"D0": 92}, "converged": True},
            {"shard_id": 2, "r_hat": {"D0": 1.01}, "ess": {"D0": 98}, "converged": True},
        ]

        cmc_result = MCMCResult(
            mean_params=np.mean(samples, axis=0),
            mean_contrast=0.5,
            mean_offset=1.0,
            samples_params=samples,
        ...
        assert cmc_result.is_cmc_result()

        # Export to ArviZ
        idata = cmc_result.to_arviz()
        assert isinstance(idata, az.InferenceData)

        # CMC metadata should be preserved in attrs if implemented
        # (This depends on MCMCResult.to_arviz() implementation)
