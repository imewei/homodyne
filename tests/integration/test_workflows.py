...
class TestEndToEndWorkflows:
    ...
    def test_synthetic_data_complete_workflow(self, temp_dir, test_config):
        ...
        if not JAXFIT_AVAILABLE:
            pytest.skip("JAXFit not available for optimization")
    ...
    def test_yaml_config_workflow(self, temp_dir):
        ...
        try:
            ...
            if result.success:
                assert hasattr(result, 'parameters')
                assert result.n_iterations <= loaded_config['optimization']['lsq']['max_iterations']
    ...
    def test_multi_q_value_workflow(self, test_config):
        ...
        if not JAXFIT_AVAILABLE or not JAX_AVAILABLE:
            pytest.skip("JAXFit or JAX not available")
        ...
        if result.success:
            ...
            assert 0.05 <= recovered_D <= 0.2, f"Diffusion coefficient recovery poor: {recovered_D}"
...
class TestModuleInteraction:
    ...
    def test_data_loader_optimization_integration(self, synthetic_xpcs_data, test_config):
    ...
    def test_backend_optimization_consistency(self, synthetic_xpcs_data, test_config):
        ...
        if not JAXFIT_AVAILABLE or not JAX_AVAILABLE:
            pytest.skip("JAX or JAXFit not available")
        ...
        if result.success:
            ...
            required_params = ['offset', 'contrast', 'diffusion_coefficient', 'shear_rate', 'L']
            ...
            backend_chi2 = chi_squared_jax(optimized_params, c2_exp, sigma, t1, t2, phi, q)
            ...
            assert relative_diff < 0.1, f"Chi-squared inconsistency: {relative_diff:.3f}"

            # Model should have reasonable values
            assert jnp.all(jnp.isfinite(model_result)), "Model result should be finite"
            assert jnp.all(model_result >= 0.8), "Model should have reasonable correlation values"
    ...
    def test_memory_behavior_consistency(self, synthetic_xpcs_data, test_config):
        ...
        if len(successful_results) >= 2:
            ...
            for key in param_keys:
                ...
                assert relative_std < 0.01, f"Parameter {key} inconsistent across runs: {relative_std:.4f}"