...
def test_weighted_gaussian_product_two_shards(gaussian_shards_2):
    ...
    for i in range(num_params):
        assert mean_0[i] <= result['mean'][i] <= mean_1[i] or \
               mean_1[i] <= result['mean'][i] <= mean_0[i]
...
def test_single_shard_edge_case():
    ...
    np.random.seed(42)
    single_shard = [
        {'samples': np.random.randn(200, 4), 'shard_id': 0}
    ]
...
def test_combine_subposteriors_end_to_end(gaussian_shards_2):
    ...
    for mean in [result_weighted['mean'], result_average['mean']]:
        for i in range(3):
            assert mean_0[i] - 1.0 <= mean[i] <= mean_1[i] + 1.0 or \
                   mean_1[i] - 1.0 <= mean[i] <= mean_0[i] + 1.0
...
def test_covariance_positive_definite():
    ...
    shards = [
        {'samples': np.random.randn(200, 4) + i}
        for i in range(3)
    ]
    ...
    assert np.all(eigenvalues_weighted > 0), \
        "Weighted covariance not positive definite"

    # Test averaging method
    result_average = _simple_averaging(shards)
    eigenvalues_average = np.linalg.eigvalsh(result_average['cov'])
    assert np.all(eigenvalues_average > 0), \
        "Averaging covariance not positive definite"
