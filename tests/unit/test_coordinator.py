...
class TestCoordinatorInitialization:
    ...
    def test_backend_selection_gpu(self, minimal_config, mock_hardware_gpu):
        """Test backend selection on GPU system."""
        with patch('homodyne.optimization.cmc.coordinator.detect_hardware') as mock_detect:
            mock_detect.return_value = mock_hardware_gpu

            coordinator = CMCCoordinator(minimal_config)

            # Should select pjit on GPU
...
class TestEndToEndPipeline:
    ...
    def test_complete_pipeline_minimal(self, minimal_config, synthetic_data, mock_nlsq_params, mock_hardware_cpu):
...
class TestIndividualSteps:
    ...
    def test_step5_basic_validation(self, minimal_config, mock_hardware_cpu):
        ...
        with patch('homodyne.optimization.cmc.coordinator.detect_hardware') as mock_detect:
            ...
            is_valid, diagnostics = coordinator._basic_validation(combined, shard_results)
...
class TestMCMCResultPackaging:
    ...
    def test_result_structure(self):
        ...
        shard_results = [
            {'samples': np.random.randn(500, 5), 'converged': True, 'acceptance_rate': 0.85},
            {'samples': np.random.randn(500, 5), 'converged': True, 'acceptance_rate': 0.82},
        ]
