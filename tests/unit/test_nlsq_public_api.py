...
class TestFitNlsqJaxAPI:
    ...
    def test_fit_nlsq_jax_api_compatibility(self):
            ...
            def __init__(self):
                self.config = {
                    ...
                    'initial_parameters': {
                        ...
                        'parameter_names': ['contrast', 'offset', 'D0', 'alpha', 'D_offset']
                    },
                    'parameter_space': {
                        'bounds': [
        ...
        assert result.success, f"Optimization should succeed with synthetic data: {result.message if hasattr(result, 'message') else 'no message'}"
        ...
        assert result_auto.success, \
            "Auto-loaded optimization should succeed"
        ...
        for param_name in param_names:
            if param_name in ground_truth and param_name in recovered:
                ...
                if true_val != 0:
                    ...
                    print(f"  {param_name}: true={true_val:.4f}, recovered={rec_val:.4f}, error={rel_error:.2%}")
