...
class TestNLSQWrapperFit:
    ...
    def test_static_isotropic_fit_small_dataset(self):
        ...
        assert result.covariance.shape == (5, 5), \
            "Covariance should be 5x5 matrix"
    ...
    def test_laminar_flow_fit_large_dataset(self):
        ...
        class MockXPCSData:
            def __init__(self):
                ...
                self.g2 = np.ones((23, 50, 50)) * 1.1 + np.random.randn(23, 50, 50) * 0.05
                self.sigma = np.ones_like(self.g2) * 0.1
                self.q = 0.01
                self.L = 1.0
                self.dt = 0.001
    ...
    def test_parameter_bounds_clipping(self):
        ...
        initial_params = np.array([1.5, 0.5, 1000.0, 0.5, 10.0])  # contrast=1.5, offset=0.5 out of bounds
...
class TestNLSQWrapperErrorRecovery:
    ...
    def test_auto_retry_on_convergence_failure(self):
        ...
        assert any('perturb' in action.lower() or 'retry' in action.lower()
                   for action in result.recovery_actions), \
            f"Recovery actions should mention perturbation/retry: {result.recovery_actions}"
        assert result.convergence_status in ['converged', 'converged_with_recovery'], \
    ...
    def test_actionable_error_diagnostics_convergence_failure(self):
        ...
        def mock_curve_fit_always_fail(f, xdata, ydata, p0, bounds):
            raise RuntimeError("Convergence failure: maximum iterations (5) reached without convergence")
        ...
        assert 'Recovery actions attempted' in error_message or 'recovery' in error_message.lower(), \
