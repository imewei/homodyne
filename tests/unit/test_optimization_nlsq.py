...
class TestNLSQOptimization:
    ...
    def test_jaxfit_availability(self):
        """Test JAXFit availability detection."""
        # This tests the import detection logic
        assert isinstance(JAXFIT_AVAILABLE, bool)
    ...
    @pytest.mark.skipif(not JAXFIT_AVAILABLE, reason="JAXFit not available")
    def test_nlsq_synthetic_data_fit(self, synthetic_xpcs_data, test_config):
        ...
        assert result.parameters['diffusion_coefficient'] >= 0.0, "Diffusion must be non-negative"
...
class TestNLSQFallback:
    """Test NLSQ fallback behavior when JAXFit is not available."""

    def test_fallback_import(self):
        """Test that module imports work without JAXFit."""
        # Should be able to import even without JAXFit
        from homodyne.optimization.nlsq import fit_nlsq_jax
        assert callable(fit_nlsq_jax)

    @pytest.mark.skipif(JAXFIT_AVAILABLE, reason="JAXFit is available")
    def test_fallback_behavior(self, synthetic_xpcs_data, test_config):
        """Test fallback behavior when JAXFit is not available."""
        data = synthetic_xpcs_data
        ...
        try:
            ...
            assert "JAXFit" in str(e) or "optimization" in str(e)
...
class TestNLSQPerformance:
    ...
    def test_nlsq_timing_small_dataset(self, small_xpcs_data, test_config, benchmark_config):
    ...
    def test_nlsq_convergence_speed(self, synthetic_xpcs_data, test_config):
        ...
        if result.success:
            ...
            if result.n_iterations < 10:
                assert result.chi_squared < 1.0, "Fast convergence should achieve good fit"