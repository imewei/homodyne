...
from homodyne.optimization.cmc.core import fit_mcmc_jax, CMCConfig
from homodyne.optimization.cmc.data_prep import PreparedData, extract_phi_info, estimate_noise_scale
from homodyne.config.parameter_space import ParameterSpace
from homodyne.optimization.cmc.plotting import plot_energy
...
def generate_synthetic_data(n_points=10000, n_phi=2):
    ...
    # Weights (unused in CMC but part of data struct usually)
    weights = np.ones_like(g2)
    
    phi_unique, phi_indices = extract_phi_info(phi)
    noise_scale = estimate_noise_scale(g2)

    return PreparedData(
        data=g2, t1=t1, t2=t2, phi=phi,
        phi_unique=phi_unique, phi_indices=phi_indices,
        n_total=n_points, n_phi=n_phi, noise_scale=noise_scale
    )
        ...
        try:
            result = fit_mcmc_jax(
                ...
                config={
                    ...
                    "per_shard_mcmc": {"num_warmup": 10, "num_samples": 10, "num_chains": 1},
            ...
            try:
                ...
                if hasattr(result, "inference_data") and hasattr(result.inference_data, "sample_stats"):
                    sample_stats = result.inference_data.sample_stats
                    # sample_stats is an xarray.Dataset
                    if "potential_energy" in sample_stats:
                         print("[PASS] 'potential_energy' is present in inference_data.sample_stats.")
                    else:
                         print(f"[FAIL] 'potential_energy' missing. Available vars: {list(sample_stats.data_vars)}")
                else:
